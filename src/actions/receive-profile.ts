...
import { getSelectedThreadIndexOrNull, getGlobalTrackOrder, getHiddenGlobalTracks, getHiddenLocalTracksByPid, getLocalTrackOrderByPid, getLegacyThreadOrder, getLegacyHiddenThreads, getShowTabOnly } from "../selectors/url-state";
import { stateFromLocation, getDataSourceFromPathParts } from "../app-logic/url-handling";
import { initializeLocalTrackOrderByPid, initializeHiddenLocalTracksByPid, computeLocalTracksByPid, computeGlobalTracks, initializeGlobalTrackOrder, initializeSelectedThreadIndex, initializeHiddenGlobalTracks, getVisibleThreads } from "../profile-logic/tracks";
import { getProfileOrNull, getProfile, getRelevantPagesForActiveTab } from "../selectors/profile";
import { getView } from "../selectors/app";
import { setDataSource } from "./profile-view";
import { computeActiveTabTracks } from "../profile-logic/active-tab";

import { FunctionsUpdatePerThread, FuncToFuncMap, RequestedLib, ImplementationFilter } from "../types/actions";
import { TransformStacksPerThread } from "../types/transforms";
import { Action, ThunkAction, Dispatch } from "../types/store";
import { Profile, ThreadIndex, IndexIntoFuncTable, BrowsingContextID } from "../types/profile";
 ...
 */
export function loadProfile(profile: Profile, config: $Shape<{
  pathInZipFile: string;
  implementationFilter: ImplementationFilter;
  transformStacks: TransformStacksPerThread;
  geckoProfiler?: $GeckoProfiler;
}> = {}, initialLoad: boolean = false): ThunkAction<Promise<void>> {
  return async dispatch => {
    if (profile.threads.length === 0) {
      console.error('This profile has no threads.', profile);
      dispatch(fatalError(new Error('No threads were captured in this profile, there is nothing to display.')));
      return;
    }
 ...
 */
export function finalizeProfileView(geckoProfiler?: $GeckoProfiler): ThunkAction<Promise<void>> {
 ...
 */
export function finalizeFullProfileView(profile: Profile, selectedThreadIndex: ThreadIndex | null, showTabOnly?: BrowsingContextID | null): ThunkAction<void> {
  return (dispatch, getState) => {
    const hasUrlInfo = selectedThreadIndex !== null;

    const globalTracks = computeGlobalTracks(profile);
    const globalTrackOrder = initializeGlobalTrackOrder(globalTracks, hasUrlInfo ? getGlobalTrackOrder(getState()) : null, getLegacyThreadOrder(getState()));
    let hiddenGlobalTracks = initializeHiddenGlobalTracks(globalTracks, profile, globalTrackOrder, hasUrlInfo ? getHiddenGlobalTracks(getState()) : null, getLegacyHiddenThreads(getState()));
    const localTracksByPid = computeLocalTracksByPid(profile);
    const localTrackOrderByPid = initializeLocalTrackOrderByPid(hasUrlInfo ? getLocalTrackOrderByPid(getState()) : null, localTracksByPid, getLegacyThreadOrder(getState()));
    let hiddenLocalTracksByPid = initializeHiddenLocalTracksByPid(hasUrlInfo ? getHiddenLocalTracksByPid(getState()) : null, localTracksByPid, profile, getLegacyHiddenThreads(getState()));
    let visibleThreadIndexes = getVisibleThreads(globalTracks, hiddenGlobalTracks, localTracksByPid, hiddenLocalTracksByPid);
    ...
    if (visibleThreadIndexes.length === 0) {
      ...
      visibleThreadIndexes = profile.threads.map((_, threadIndex) => threadIndex);
    ...
    selectedThreadIndex = initializeSelectedThreadIndex(selectedThreadIndex, visibleThreadIndexes, profile);
    ...
    for (const [pid, localTracks] of localTracksByPid) {
      ...
      if (hiddenLocalTracks.size === localTracks.length) {
        ...
        const globalTrackIndex = globalTracks.findIndex(globalTrack => globalTrack.type === 'process' && globalTrack.pid === pid && globalTrack.mainThreadIndex === null);
 ...
 * view information, this function will compute the defaults.
 */
export function finalizeActiveTabProfileView(profile: Profile, selectedThreadIndex: ThreadIndex, showTabOnly?: BrowsingContextID | null): ThunkAction<void> {
  return (dispatch, getState) => {
    const relevantPages = getRelevantPagesForActiveTab(getState());
    const {
      globalTracks,
      resourceTracks
    } = computeActiveTabTracks(profile, relevantPages, getState());
 ...
 */
export function changeViewAndRecomputeProfileData(showTabOnly: BrowsingContextID | null): ThunkAction<void> {
  return (dispatch, getState) => {
    ...
    if (showTabOnly === null) {
      // The url state says this is a full view. We should compute and initialize
      // the state relevant to that state.
      dispatch(finalizeFullProfileView(profile, selectedThreadIndex, showTabOnly));
    } else {
      // The url state says this is an active tab view. We should compute and
      // initialize the state relevant to that state.
      dispatch(finalizeActiveTabProfileView(profile, selectedThreadIndex, showTabOnly));
    }
  };
}
...
export function viewProfile(profile: Profile, config: $Shape<{
  pathInZipFile: string;
  implementationFilter: ImplementationFilter;
  transformStacks: TransformStacksPerThread;
  geckoProfiler: $GeckoProfiler;
...
let requestIdleCallbackPolyfill: (callback: () => void, _opts?: {timeout: number;}) => unknown;
...
export function mergeFunctions(threadIndex: ThreadIndex, oldFuncToNewFuncMap: FuncToFuncMap): ThunkAction<void> {
  return dispatch => {
    gCoalescedFunctionsUpdateDispatcher.mergeFunctions(dispatch, threadIndex, oldFuncToNewFuncMap);
  };
}

export function assignFunctionNames(threadIndex: ThreadIndex, funcIndices: IndexIntoFuncTable[], funcNames: string[]): ThunkAction<void> {
  return dispatch => {
...
async function getProfileFromAddon(dispatch: Dispatch, geckoProfiler: $GeckoProfiler): Promise<Profile> {
...
function getSymbolStore(dispatch: Dispatch, geckoProfiler?: $GeckoProfiler): SymbolStore | null {
  if (!window.indexedDB) {
  ...
  const symbolStore = new SymbolStore('perf-html-async-storage', {
    requestSymbolsFromServer: requests => {
      ...
      return MozillaSymbolicationAPI.requestSymbols(requests).map(async (libPromise, i) => {
        try {
...
export async function doSymbolicateProfile(dispatch: Dispatch, profile: Profile, symbolStore: SymbolStore) {
  ...
  return async dispatch => {
    try {
      const timeoutId = setTimeout(() => {
        dispatch(temporaryError(new TemporaryError(oneLine`
...
type FetchProfileArgs = {
  url: string;
  onTemporaryError: (arg0: TemporaryError) => void;
 ...
 */
export async function _fetchProfile(args: FetchProfileArgs): Promise<ProfileOrZip> {
  ...
  while (true) {
    ...
    onTemporaryError(new TemporaryError('Profile not found on remote server.', { count: i, total: MAX_WAIT_SECONDS + 1 } // 11 tries during 10 seconds
    ));
 ...
 */
function _deduceContentType(url: string, contentType: string | null): "application/json" | "application/zip" | null {
 ...
 */
async function _extractProfileOrZipFromResponse(url: string, response: Response, reportError: Function): Promise<ProfileOrZip> {
  const contentType = _deduceContentType(url, response.headers.get('content-type'));
  switch (contentType) {
    case 'application/zip':
      return {
        zip: await _extractZipFromResponse(response, reportError)
      };
    case 'application/json':case null:
      // The content type is null if it is unknown, or an unsupported type. Go ahead
      // and try to process it as a profile.
      return {
        profile: await _extractJsonFromResponse(response, reportError, contentType)
      };
    default:
 ...
 */
async function _extractZipFromResponse(response: Response, reportError: Function): Promise<JSZip> {
 ...
 */
async function _extractJsonFromResponse(response: Response, reportError: Function, fileType: "application/json" | null): Promise<any> {
  try {
...
export function retrieveProfileFromStore(hash: string, initialLoad: boolean = false): ThunkAction<Promise<void>> {
 ...
 */
export function retrieveProfileOrZipFromUrl(profileUrl: string, initialLoad: boolean = false): ThunkAction<Promise<void>> {
  return async function (dispatch) {
    ...
    try {
      ...
      if (serializedProfile) {
        ...
        throw new Error('Expected to receive a zip file or profile from _fetchProfile.');
      }
    } catch (error) {
      dispatch(fatalError(error));
    }
...
function _fileReader(input: File): any {
 ...
 */
export function retrieveProfileFromFile(file: File, // Allow tests to inject a custom file reader to bypass the DOM APIs.
fileReader: typeof _fileReader = _fileReader): ThunkAction<Promise<void>> {
  return async dispatch => {
    ...
    try {
      switch (file.type) {
        case 'application/gzip':case 'application/x-gzip':
 ...
 */
export function retrieveProfilesToCompare(profileViewUrls: string[], initialLoad: boolean = false): ThunkAction<Promise<void>> {
  return async dispatch => {
    ...
    try {
      ...
      const profileStates = await Promise.all(profileViewUrls.map(async url => {
        if (url.startsWith('https://perfht.ml/') || url.startsWith('https://bit.ly/')) {
          url = await expandUrl(url);
        }
        return stateFromLocation(new URL(url));
      }));

      const hasSupportedDatasources = profileStates.every(state => state.dataSource === 'public');
      ...
      const promises = profileStates.map(async ({
        hash
      }) => {
      ...
      await dispatch(loadProfile(resultProfile, {
        transformStacks,
...
export function getProfilesFromRawUrl(location: Location): ThunkAction<Promise<{profile: Profile | null;shouldSetupInitialUrlState: boolean;}>> {
  return async (dispatch, getState) => {
    ...
    switch (dataSource) {
      case 'from-addon':
        shouldSetupInitialUrlState = false; // We don't need to `await` the result because there's no url upgrading
      ...
      case 'public':
        ...
        break;
      case 'from-url':
        await dispatch(retrieveProfileOrZipFromUrl(decodeURIComponent(pathParts[1]), true));
        break;
      case 'compare':
        {
      ...
      case 'none':case 'from-file':case 'local':
        throw new Error(`There is no profile to download`);
      default:
        throw assertExhaustiveCheck(dataSource, `Unknown dataSource ${dataSource}.`);
