...
export function addDataToWindowObject(getState: GetState, dispatch: Dispatch, target: Object = window) {
  ...
  defineProperty(target, 'filteredThread', {
    enumerable: true,
    get() {
      return selectorsForConsole.selectedThread.getPreviewFilteredThread(getState());
    }
  });
  ...
  defineProperty(target, 'filteredMarkers', {
    ...
    get() {
      ...
      const getMarker = selectorsForConsole.selectedThread.getMarkerGetter(state);
      const markerIndexes = selectorsForConsole.selectedThread.getPreviewFilteredMarkerIndexes(state);
...
export function logFriendlyPreamble() {
  ...
  const reset = '';
  console.log( // This is gratuitous, I know:
  ['%c  __ _           __                                  ', ' / _(_)         / _|                                 ', '| |_ _ _ __ ___| |_ _____  __                        ', "|  _| | '__/ _ \\  _/ _ \\ \\/ /                        ", '| | | | | |  __/ || (_) >  <       __ _ _            ', '|_| |_|_|  \\___|_| \\___/_/\\_\\     / _(_) |           ', '    ,.       ,.   _ __  _ __ ___ | |_ _| | ___ _ _   ', "    | \\     / |  | '_ \\| '__/ _ \\|  _| | |/ _ \\ '_|  ", '    |/ \\ _ / \\|  | |_) | | | (_) | | | | |  __/ |    ', '    |         |  | .__/|_|  \\___/|_| |_|_|\\___|_|    ', '    /  -    - \\  |_|                                     ', '  ,-    V__V   -.                                     ', ' -=  __-  * - .,=-                                    ', '  `\\_    -   _/                                       ', "      `-----'                                         "].join('\n'), 'font-family: Menlo, monospace;');
  ...
  intro, reset, // "window.profile"
  bold, reset, // "window.filteredThread"
  bold, reset, // "window.filteredMarkers"
  bold, reset, // "window.callTree"
  bold, reset, // "window.getState"
  bold, reset, // "window.selectors"
  bold, reset, // "window.dispatch"
  bold, reset, // "window.actions"
  bold, reset, // "processed-profile-format.md"
  link, reset, // "call-tree.js"
  link, reset);
}