...
function subtractPadding(element: HTMLElement, rect: DOMRect): DOMRect {
  ...
  // $FlowFixMe Error introduced by upgrading to v0.96.0. See issue #1935
  return new DOMRect(rect.left + paddingLeft, rect.top + paddingTop, rect.width - paddingLeft - paddingRight, rect.height - paddingTop - paddingBottom);
}
...
function addMargin(element: HTMLElement, rect: DOMRect): DOMRect {
  ...
  // $FlowFixMe Error introduced by upgrading to v0.96.0. See issue #1935
  return new DOMRect(rect.left - marginLeft, rect.top - marginTop, rect.width + marginLeft + marginRight, rect.height + marginTop + marginBottom);
}
...
export function getContentRect(element: HTMLElement): DOMRect {
  ...
  return subtractPadding(element, subtractBorder(element, clientRectToDomRect(borderRect)));
}
 ...
 */
export function extractDomRectValue(rect: DOMRect, key: "top" | "left" | "right" | "bottom"): number {
  return (rect as Object)[key];
}