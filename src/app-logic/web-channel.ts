import { $Values, $Keys, $PropertyType } from "utility-types";
...
export async function queryIsMenuButtonEnabled(): Promise<boolean> {
  type ExpectedResponse = $PropertyType<MessageFromBrowserObject, "STATUS_RESPONSE">;
...
export async function enableMenuButton(): Promise<void> {
  type ExpectedResponse = $PropertyType<MessageFromBrowserObject, "ENABLE_MENU_BUTTON_DONE">;
...
function _sendMessageWithResponse<Returns extends MessageFromBrowser>(messageToBrowser: MessageToBrowser): Promise<Returns> {
  return new Promise((resolve, reject) => {
    function listener(event) {
      ...
      if (id === 'profiler.firefox.com' && messageFromBrowser && typeof messageFromBrowser === 'object') {
        if (typeof messageFromBrowser.error === 'string') {
          ...
          console.error(`[webchannel] %c${messageFromBrowser.error}`, LOG_STYLE);
          window.removeEventListener('WebChannelMessageToContent', listener, true);
          reject(messageFromBrowser);
        } else if (messageToBrowser.requestId === messageFromBrowser.requestId) {
          if (process.env.NODE_ENV === 'development') {
            console.log(`[webchannel] %creceived "${String(messageFromBrowser.type)}"`, LOG_STYLE, messageFromBrowser);
          }
          window.removeEventListener('WebChannelMessageToContent', listener, true);

          resolve(( // Make the type system assume that we have the right message.
