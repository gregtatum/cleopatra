...
import { stringifyCommittedRanges, parseCommittedRanges } from "../profile-logic/committed-ranges";
import { stringifyTransforms, parseTransforms } from "../profile-logic/transforms";
import { assertExhaustiveCheck, toValidTabSlug, ensureExists } from "../utils/flow";
...
function getDataSourceDirs(urlState: UrlState): [] | [DataSource] | [DataSource, string] {
  const {
    dataSource
  } = urlState;
  switch (dataSource) {
...
type BaseQuery = FullProfileSpecificBaseQuery & ActiveTabProfileSpecificBaseQuery & {
  v: number;
  range: string;
  //
  thread: string;
...
type Query = CallTreeQuery | MarkersQuery | NetworkQuery | StackChartQuery | JsTracerQuery;

type $MakeOptional = <T>(arg0: T) => T | void;
...
type FullProfileSpecificBaseQueryShape = $Shape<$ObjMap<FullProfileSpecificBaseQuery, $MakeOptional>>;
type ActiveTabProfileSpecificBaseQueryShape = $Shape<$ObjMap<ActiveTabProfileSpecificBaseQuery, $MakeOptional>>;
...
type QueryShape = CallTreeQueryShape | MarkersQueryShape | NetworkQueryShape | StackChartQueryShape | JsTracerQueryShape;
...
export function urlStateToUrlObject(urlState: UrlState): UrlObject {
  ...
  if (urlState.showTabOnly === null) {
    ...
    baseQuery = ({} as FullProfileSpecificBaseQueryShape);
    baseQuery.globalTrackOrder = urlState.profileSpecific.full.globalTrackOrder.join('-') || undefined;

    // Add the parameter hiddenGlobalTracks only when needed.
    if (urlState.profileSpecific.full.hiddenGlobalTracks.size > 0) {
      baseQuery.hiddenGlobalTracks = [...urlState.profileSpecific.full.hiddenGlobalTracks].join('-');
    }

    let hiddenLocalTracksByPid = '';
    for (const [pid, tracks] of urlState.profileSpecific.full.hiddenLocalTracksByPid) {
      if (tracks.size > 0) {
    ...
    for (const [pid, trackOrder] of urlState.profileSpecific.full.localTrackOrderByPid) {
    ...
    baseQuery = ({} as ActiveTabProfileSpecificBaseQueryShape);
  }

  baseQuery = ({
    ...baseQuery,
    range: stringifyCommittedRanges(urlState.profileSpecific.committedRanges) || undefined,
  ...
  switch (selectedTab) {
    case 'stack-chart':case 'flame-graph':case 'calltree':
      {
        if (selectedTab === 'stack-chart') {
          ...
          query.showUserTimings = urlState.profileSpecific.showUserTimings ? null : undefined;
        } else {
          query = (baseQuery as CallTreeQueryShape);
        }

        query.search = urlState.profileSpecific.callTreeSearchString || undefined;
        query.invertCallstack = urlState.profileSpecific.invertCallstack ? null : undefined;
        query.implementation = urlState.profileSpecific.implementation === 'combined' ? undefined : urlState.profileSpecific.implementation;
        if (selectedThread !== null) {
          query.transforms = stringifyTransforms(urlState.profileSpecific.transforms[selectedThread]) || undefined;
        }
        query.ctSummary = urlState.profileSpecific.lastSelectedCallTreeSummaryStrategy === 'timing' ? undefined : urlState.profileSpecific.lastSelectedCallTreeSummaryStrategy;
        break;
      }
    case 'marker-table':case 'marker-chart':
      query = (baseQuery as MarkersQueryShape);
      query.markerSearch = urlState.profileSpecific.markersSearchString || undefined;
      break;
    case 'network-chart':
      query = (baseQuery as NetworkQueryShape);
      query.networkSearch = urlState.profileSpecific.networkSearchString || undefined;
      break;
    case 'js-tracer':
      ...
      if (urlState.showTabOnly === null) {
        ...
        query.summary = urlState.profileSpecific.full.showJsTracerSummary ? null : undefined;
      }
...
export function getDataSourceFromPathParts(pathParts: string[]): DataSource {
  ...
  switch (str) {
    case 'none':case 'from-addon':case 'from-file':case 'local':case 'public':case 'from-url':case 'compare':
      return str;
    default:
...
export function stateFromLocation(location: Location, profile?: Profile): UrlState {
  const {
    pathname,
    query
  ...
  const transforms = {};
  if (selectedThread !== null) {
    transforms[selectedThread] = query.transforms ? parseTransforms(query.transforms) : [];
  }
  ...
  return {
    ...
    profileSpecific: {
      implementation,
      lastSelectedCallTreeSummaryStrategy: toValidCallTreeSummaryStrategy(query.ctSummary || undefined),
      ...
      full: {
        ...
        globalTrackOrder: query.globalTrackOrder ? query.globalTrackOrder.split('-').map(index => Number(index)) : [],
        hiddenGlobalTracks: query.hiddenGlobalTracks ? new Set(query.hiddenGlobalTracks.split('-').map(index => Number(index))) : new Set(),
        hiddenLocalTracksByPid: query.hiddenLocalTracksByPid ? parseHiddenTracks(query.hiddenLocalTracksByPid) : new Map(),
        localTrackOrderByPid: query.localTrackOrderByPid ? parseLocalTrackOrder(query.localTrackOrderByPid) : new Map(),
        timelineType: query.timelineType === 'stack' ? 'stack' : 'category',
        legacyThreadOrder: query.threadOrder ? query.threadOrder.split('-').map(index => Number(index)) : null,
        legacyHiddenThreads: query.hiddenThreads ? query.hiddenThreads.split('-').map(index => Number(index)) : null
      }
...
export function upgradeLocationToCurrentVersion(processedLocation: ProcessedLocationBeforeUpgrade, profile?: Profile): ProcessedLocation {
  ...
  if (urlVersion > CURRENT_URL_VERSION) {
    throw new Error(`Unable to parse a url of version ${urlVersion}, most likely profiler.firefox.com needs to be refreshed. ` + `The most recent version understood by this version of profiler.firefox.com is version ${CURRENT_URL_VERSION}.\n` + 'You can try refreshing this page in case profiler.firefox.com has updated in the meantime.');
  }
  // Convert to CURRENT_URL_VERSION, one step at a time.
  for (let destVersion = urlVersion; destVersion <= CURRENT_URL_VERSION; destVersion++) {
...
const _upgraders = {
  [0]: (processedLocation: ProcessedLocationBeforeUpgrade, _: Profile) => {
    ...
    if (processedLocation.pathname === '/') {
      const legacyQuery = Object.assign({}, processedLocation.query, queryString.parse(processedLocation.hash));
  ...
  [1]: (processedLocation: ProcessedLocationBeforeUpgrade, _: Profile) => {
    ...
    if (processedLocation.query.callTreeFilters) {
      ...
      processedLocation.query.transforms = processedLocation.query.callTreeFilters.split('~').map(s => {
  ...
  [2]: (processedLocation: ProcessedLocationBeforeUpgrade, _: Profile) => {
    ...
    processedLocation.pathname = processedLocation.pathname // Given:    /public/e71ce9584da34298627fb66ac7f2f245ba5edbf5/timeline/
    // Matches:  $1^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    .replace(/^(\/[^/]+\/[^/]+)\/timeline\/?/, '$1/stack-chart/') // Given:    /public/e71ce9584da34298627fb66ac7f2f245ba5edbf5/markers/
    // Matches:  $1^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  [4]: (processedLocation: ProcessedLocationBeforeUpgrade, profile: Profile) => {
    ...
    const selectedThread = query.thread !== undefined ? +query.thread : null;
    const transforms = query.transforms ? parseTransforms(query.transforms) : [];
    ...
    for (let i = 0; i < transforms.length; i++) {
      ...
      if (!transform.implementation || transform.implementation !== 'js' || !transform.callNodePath) {
        // Only transforms with JS implementation filters that have callNodePaths
        // need to be upgraded.
        continue;
      }

      const callNodeStackIndex = getStackIndexFromVersion3JSCallNodePath(thread, transform.callNodePath);
      if (callNodeStackIndex === null) {
        // If we can't find the stack index of given call node path, just abort.
        continue;
      }
      transform.callNodePath = getVersion4JSCallNodePathFromStackIndex(thread, callNodeStackIndex);
    }
...
function getStackIndexFromVersion3JSCallNodePath(thread: Thread, oldCallNodePath: CallNodePath): IndexIntoStackTable | null {
  const {
    stackTable,
    funcTable,
    frameTable
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    ...
    const doesPrefixMatchCallNodePath = prefix === null || stackIndexDepth.has(prefix);

    if (!doesPrefixMatchCallNodePath) {
      continue;
    }
    const prefixStackDepth = ensureExists(stackIndexDepth.get(prefix), 'Unable to find the stack depth for a prefix');
...
function getVersion4JSCallNodePathFromStackIndex(thread: Thread, stackIndex: IndexIntoStackTable): CallNodePath {
  const {
    funcTable,
    stackTable,
    frameTable
