...
import { DataSource, ImplementationFilter, CallTreeSummaryStrategy, TimelineType } from "../types/actions";
import { UrlState, Reducer } from "../types/state";
...
const selectedTab: Reducer<TabSlug> = (state = 'calltree', action) => {
  switch (action.type) {
    case 'CHANGE_SELECTED_TAB':case 'SELECT_TRACK':
...
const selectedThread: Reducer<ThreadIndex | null> = (state = null, action) => {
  switch (action.type) {
    case 'CHANGE_SELECTED_THREAD':case 'SELECT_TRACK':case 'VIEW_FULL_PROFILE':case 'VIEW_ACTIVE_TAB_PROFILE':case 'ISOLATE_PROCESS':case 'ISOLATE_PROCESS_MAIN_THREAD':case 'HIDE_GLOBAL_TRACK':case 'HIDE_LOCAL_TRACK':case 'ISOLATE_LOCAL_TRACK':
      // Only switch to non-null selected threads.
      return (action.selectedThreadIndex as ThreadIndex);
    case 'SANITIZED_PROFILE_PUBLISHED':
      {
        const {
          oldThreadIndexToNew
 ...
 */
const implementation: Reducer<ImplementationFilter> = (state = 'combined', action) => {
  switch (action.type) {
    case 'PROFILE_LOADED':
 ...
 */
const lastSelectedCallTreeSummaryStrategy: Reducer<CallTreeSummaryStrategy> = (state = 'timing', action) => {
  switch (action.type) {
...
const globalTrackOrder: Reducer<TrackIndex[]> = (state = [], action) => {
  switch (action.type) {
    case 'VIEW_FULL_PROFILE':case 'CHANGE_GLOBAL_TRACK_ORDER':
...
const hiddenGlobalTracks: Reducer<Set<TrackIndex>> = (state = new Set(), action) => {
  switch (action.type) {
    case 'VIEW_FULL_PROFILE':case 'ISOLATE_LOCAL_TRACK':case 'ISOLATE_PROCESS':case 'ISOLATE_PROCESS_MAIN_THREAD':case 'ISOLATE_SCREENSHOT_TRACK':
...
const hiddenLocalTracksByPid: Reducer<Map<Pid, Set<TrackIndex>>> = (state = new Map(), action) => {
  switch (action.type) {
    ...
    case 'ISOLATE_PROCESS_MAIN_THREAD':case 'ISOLATE_LOCAL_TRACK':
      {
        const hiddenLocalTracksByPid = new Map(state);
...
const localTrackOrderByPid: Reducer<Map<Pid, TrackIndex[]>> = (state = new Map(), action) => {
  switch (action.type) {
    case 'VIEW_FULL_PROFILE':
...
const showTabOnly: Reducer<BrowsingContextID | null> = (state = null, action) => {
  switch (action.type) {
 ...
 */
const wrapReducerInResetter = (regularUrlStateReducer: Reducer<UrlState>): Reducer<UrlState> => {
  return (state, action) => {
    switch (action.type) {
      case 'UPDATE_URL_STATE':
        ...
        // in the history API.
        return action.newUrlState ? action.newUrlState : regularUrlStateReducer(undefined, action);
      case 'RETURN_TO_ZIP_FILE_LIST':case 'WAITING_FOR_PROFILE_FROM_FILE':
...
const urlStateReducer: Reducer<UrlState> = wrapReducerInResetter(combineReducers({
  dataSource,
  hash,
  profileUrl,
  profilesToCompare,
