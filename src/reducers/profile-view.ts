...
import { applyFunctionMerging, setFuncNames } from "../profile-logic/symbolication";
import { combineReducers } from "redux";
import * as Transforms from "../profile-logic/transforms";
import * as ProfileData from "../profile-logic/profile-data";
import { arePathsEqual, PathSet } from "../utils/path";

import { Profile, Pid } from "../types/profile";
import { LocalTrack, GlobalTrack, TrackIndex, ActiveTabGlobalTrack } from "../types/profile-derived";
import { StartEndRange } from "../types/units";
import { PreviewSelection, RequestedLib, TrackReference } from "../types/actions";
import { Reducer, ProfileViewState, SymbolicationStatus, ThreadViewOptions, RightClickedCallNode, RightClickedMarker } from "../types/state";
...
const profile: Reducer<Profile | null> = (state = null, action) => {
  switch (action.type) {
    ...
    case 'COALESCED_FUNCTIONS_UPDATE':
      {
        if (state === null) {
          throw new Error('Assumed that a profile would be loaded in time for a coalesced functions update.');
        }
        ...
        const threads = state.threads.map((thread, threadIndex) => {
          ...
          return setFuncNames(applyFunctionMerging(thread, oldFuncToNewFuncMap), funcIndices, funcNames);
        });
    ...
    case 'DONE_SYMBOLICATING':
      {
        if (state === null) {
          throw new Error(`Strangely we're done symbolicating a non-existent profile.`);
        }
 ...
 * for the same reason.
 */
const localTracksByPid: Reducer<Map<Pid, LocalTrack[]>> = (state = new Map(), action) => {
  switch (action.type) {
    case 'VIEW_FULL_PROFILE':
 ...
 * pass. It is valid for the lifetime of the profile.
 */
const activeTabGlobalTracks: Reducer<ActiveTabGlobalTrack[]> = (state = [], action) => {
  switch (action.type) {
 ...
 * pass. It is valid for the lifetime of the profile.
 */
const activeTabHiddenGlobalTracksGetter: Reducer<() => Set<TrackIndex>> = (state = () => new Set(), action) => {
  switch (action.type) {
    default:
      return state;
 ...
 * for the same reason.
 */
const activeTabHiddenLocalTracksByPidGetter: Reducer<() => Map<Pid, Set<TrackIndex>>> = (state = () => new Map(), action) => {
  switch (action.type) {
    default:
      return state;
...
const symbolicationStatus: Reducer<SymbolicationStatus> = (state = 'DONE', action) => {
  switch (action.type) {
...
const viewOptionsPerThread: Reducer<ThreadViewOptions[]> = (state = [], action) => {
  switch (action.type) {
    case 'PROFILE_LOADED':
    ...
    case 'COALESCED_FUNCTIONS_UPDATE':
      {
        ...
        return state.map((threadViewOptions, threadIndex) => {
          ...
          return {
            ...threadViewOptions,
            selectedCallNodePath: threadViewOptions.selectedCallNodePath.map(mapOldFuncToNewFunc),
            expandedCallNodePaths: new PathSet(Array.from(threadViewOptions.expandedCallNodePaths).map(oldPath => oldPath.map(mapOldFuncToNewFunc)))
          };
        });
      }
    case 'CHANGE_SELECTED_CALL_NODE':
      {
        ...
        if (arePathsEqual(selectedCallNodePath, previousSelectedCallNodePath) && !optionalExpandedToCallNodePath) {
          return state;
        }
        ...
        const expandToNode = optionalExpandedToCallNodePath ? optionalExpandedToCallNodePath : selectedCallNodePath;
        ...
        const hasNewExpandedPaths = expandToNodeParentPaths.some(path => !expandedCallNodePaths.has(path));

        if (hasNewExpandedPaths) {
          expandedCallNodePaths = new PathSet(expandedCallNodePaths);
          expandToNodeParentPaths.forEach(path => expandedCallNodePaths.add(path));
        }

        return [...state.slice(0, threadIndex), {
          ...state[threadIndex],
          selectedCallNodePath,
          expandedCallNodePaths
    ...
    case 'CHANGE_INVERT_CALLSTACK':
      {
        ...
        return state.map((viewOptions, threadIndex) => {
          if (selectedThreadIndex === threadIndex) {
            ...
            const selectedCallNodePath = Transforms.invertCallNodePath(viewOptions.selectedCallNodePath, callTree, callNodeTable);
    ...
    case 'CHANGE_SELECTED_MARKER':
      {
        ...
        return [...state.slice(0, threadIndex), { ...state[threadIndex], selectedMarker }, ...state.slice(threadIndex + 1)];
      }
    case 'ADD_TRANSFORM_TO_STACK':
      {
        ...
        const expandedCallNodePaths = new PathSet(Array.from(state[threadIndex].expandedCallNodePaths).map(path => Transforms.applyTransformToCallNodePath(path, transform, transformedThread)).filter(path => path.length > 0));

        const selectedCallNodePath = Transforms.applyTransformToCallNodePath(state[threadIndex].selectedCallNodePath, transform, transformedThread);
    ...
    case 'CHANGE_IMPLEMENTATION_FILTER':
      {
        ...
        if (implementation === 'combined') {
          ...
          selectedCallNodePath = Transforms.restoreAllFunctionsInCallNodePath(transformedThread, previousImplementation, selectedCallNodePath);
        } else {
          ...
          selectedCallNodePath = Transforms.filterCallNodePathByImplementation(transformedThread, implementation, selectedCallNodePath);
        }
...
const waitingForLibs: Reducer<Set<RequestedLib>> = (state = new Set(), action) => {
  switch (action.type) {
...
const previewSelection: Reducer<PreviewSelection> = (state = { hasSelection: false, isModifying: false }, action) => {
  switch (action.type) {
...
const scrollToSelectionGeneration: Reducer<number> = (state = 0, action) => {
  switch (action.type) {
    case 'CHANGE_INVERT_CALLSTACK':case 'CHANGE_SELECTED_CALL_NODE':case 'CHANGE_SELECTED_THREAD':case 'SELECT_TRACK':case 'HIDE_GLOBAL_TRACK':case 'HIDE_LOCAL_TRACK':case 'CHANGE_SELECTED_MARKER':
      return state + 1;
    default:
      return state;
...
const rootRange: Reducer<StartEndRange> = (state = { start: 0, end: 1 }, action) => {
  switch (action.type) {
    case 'PROFILE_LOADED':
...
const rightClickedTrack: Reducer<TrackReference | null> = (state = null, action) => {
  switch (action.type) {
...
const rightClickedCallNode: Reducer<RightClickedCallNode | null> = (state = null, action) => {
  switch (action.type) {
    case 'COALESCED_FUNCTIONS_UPDATE':
      {
        const {
    ...
    case 'SET_CONTEXT_MENU_VISIBILITY':
      ...
      return null;
    case 'PROFILE_LOADED':case 'CHANGE_INVERT_CALLSTACK':case 'ADD_TRANSFORM_TO_STACK':case 'POP_TRANSFORMS_FROM_STACK':case 'CHANGE_IMPLEMENTATION_FILTER':
      return null;
    default:
      return state;
...
const rightClickedMarker: Reducer<RightClickedMarker | null> = (state = null, action) => {
  switch (action.type) {
 ...
 * (e.g. in zip files).
 */
const wrapReducerInResetter = (regularReducer: Reducer<ProfileViewState>): Reducer<ProfileViewState> => {
  return (state, action) => {
    switch (action.type) {
      case 'SANITIZED_PROFILE_PUBLISHED':case 'REVERT_TO_PRE_PUBLISHED_STATE':case 'RETURN_TO_ZIP_FILE_LIST':
...
const profileViewReducer: Reducer<ProfileViewState> = wrapReducerInResetter(combineReducers({
