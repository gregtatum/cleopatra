...
import { ZipFileState, Reducer, ZippedProfilesState } from "../types/state";

/**
 * This reducer contains all of the state that deals with loading in profiles from
 * zip files.
 */
const zipFile: Reducer<ZipFileState> = (state = {
  phase: 'NO_ZIP_FILE',
  zip: null,
  pathInZipFile: null
}, action) => {
  switch (action.type) {
    case 'UPDATE_URL_STATE':
      {
        if (action.newUrlState && action.newUrlState.pathInZipFile === null && state.phase === 'VIEW_PROFILE_IN_ZIP_FILE') {
    ...
    case 'RETURN_TO_ZIP_FILE_LIST':case 'DISMISS_PROCESS_PROFILE_FROM_ZIP_ERROR':
      return _validateStateTransition(state, {
        phase: 'LIST_FILES_IN_ZIP_FILE',
    ...
    case 'FATAL_ERROR':
      return state.phase === 'NO_ZIP_FILE' || state.phase === 'LIST_FILES_IN_ZIP_FILE' ? state : _validateStateTransition(state, {
        phase: 'FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE',
        zip: ensureExists(state.zip),
        pathInZipFile: ensureExists(state.pathInZipFile)
      });
    case 'VIEW_FULL_PROFILE':case 'VIEW_ACTIVE_TAB_PROFILE':
      ...
      return state.phase === 'NO_ZIP_FILE' ? state : _validateStateTransition(state, {
        phase: 'VIEW_PROFILE_IN_ZIP_FILE',
        zip: ensureExists(state.zip),
        pathInZipFile: ensureExists(state.pathInZipFile)
      });
    case 'PROFILE_PUBLISHED':case 'SANITIZED_PROFILE_PUBLISHED':case 'WAITING_FOR_PROFILE_FROM_FILE':
...
const error: Reducer<null | Error> = (state = null, action) => {
  switch (action.type) {
    case 'FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE':case 'FATAL_ERROR':
      return action.error;
    default:
      return state;
 ...
 */
function _validateStateTransition(prev: ZipFileState, next: ZipFileState): ZipFileState {
  ...
  switch (prevPhase) {
    case 'NO_ZIP_FILE':
      expectedNextPhases = [// Coming into a fresh page load with a zip file, this is the first state
      // transition.
      'LIST_FILES_IN_ZIP_FILE', // This will happen if coming in from a URL that is already pointing at a file
      // in a zip.
      'PROCESS_PROFILE_FROM_ZIP_FILE'];
      break;
    case 'LIST_FILES_IN_ZIP_FILE':
      expectedNextPhases = ['PROCESS_PROFILE_FROM_ZIP_FILE', 'FILE_NOT_FOUND_IN_ZIP_FILE'];
      break;
    case 'PROCESS_PROFILE_FROM_ZIP_FILE':
      expectedNextPhases = ['VIEW_PROFILE_IN_ZIP_FILE', 'FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE', // When navigating with the URL, it's possible to go back and list the files.
    ...
    case 'VIEW_PROFILE_IN_ZIP_FILE':
      ...
      break;
    default:
      throw new Error(`Unhandled ZipFileState “${(prevPhase as never)}”`);
...
const selectedZipFileIndex: Reducer<null | ZipFiles.IndexIntoZipFileTable> = (state = null, action) => {
  switch (action.type) {
...
const expandedZipFileIndexes: Reducer<Array<ZipFiles.IndexIntoZipFileTable | null>> = ( // In practice this should never contain null, but needs to support the
// TreeView interface.
state = [], action) => {
  switch (action.type) {
