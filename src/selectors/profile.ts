...
import { filterCounterToRange, accumulateCounterSamples, extractProfileFilterPageData } from "../profile-logic/profile-data";
import { IPCMarkerCorrelations, correlateIPCMarkers } from "../profile-logic/marker-data";

import { Profile, CategoryList, IndexIntoCategoryList, Thread, ThreadIndex, Pid, Counter, CounterIndex, PageList, ProfileMeta, VisualMetrics, ProgressGraphData, ProfilerConfiguration, InnerWindowID, BrowsingContextID, Page } from "../types/profile";
import { LocalTrack, TrackIndex, GlobalTrack, AccumulatedCounterSamples, ProfileFilterPageData, ActiveTabGlobalTrack } from "../types/profile-derived";
import { Milliseconds, StartEndRange } from "../types/units";
import { GlobalTrackReference, LocalTrackReference, TrackReference, PreviewSelection, HiddenTrackCount } from "../types/actions";
import { Selector, DangerousSelectorWithArguments } from "../types/store";
import { State, ProfileViewState, SymbolicationStatus, FullProfileViewState, ActiveTabProfileViewState } from "../types/state";
import { $ReturnType } from "../types/utils";
 ...
 */
export const getProfileViewOptions: Selector<any> = state => getProfileView(state).viewOptions;
export const getProfileRootRange: Selector<StartEndRange> = state => getProfileViewOptions(state).rootRange;
export const getSymbolicationStatus: Selector<SymbolicationStatus> = state => getProfileViewOptions(state).symbolicationStatus;
export const getScrollToSelectionGeneration: Selector<number> = state => getProfileViewOptions(state).scrollToSelectionGeneration;
export const getFocusCallTreeGeneration: Selector<number> = state => getProfileViewOptions(state).focusCallTreeGeneration;
export const getZeroAt: Selector<Milliseconds> = state => getProfileRootRange(state).start;

export const getCommittedRange: Selector<StartEndRange> = createSelector(getProfileRootRange, getZeroAt, UrlState.getAllCommittedRanges, (rootRange, zeroAt, committedRanges): StartEndRange => {
  if (committedRanges.length > 0) {
    let {
      start,
      end
    } = committedRanges[committedRanges.length - 1];
    start += zeroAt;
    end += zeroAt;
    return { start, end };
  }
 ...
 */
export const getPreviewSelectionRange: Selector<StartEndRange> = createSelector(getCommittedRange, getPreviewSelection, (committedRange, previewSelection) => {
...
export const getProfile: Selector<Profile> = state => ensureExists(getProfileOrNull(state), 'Tried to access the profile before it was loaded.');
...
export const getCategories: Selector<CategoryList> = state => getProfile(state).meta.categories;
export const getDefaultCategory: Selector<IndexIntoCategoryList> = state => getCategories(state).findIndex(c => c.color === 'grey');
...
export const getRightClickedTrack: Selector<TrackReference | null> = state => getProfileViewOptions(state).rightClickedTrack;
...
export const getVisualMetrics: Selector<VisualMetrics> = state => ensureExists(getVisualMetricsOrNull(state), 'Tried to access the visual metrics when it does not exist.');
export const getVisualProgress: Selector<ProgressGraphData[]> = state => getVisualMetrics(state).VisualProgress;
export const getPerceptualSpeedIndexProgress: Selector<ProgressGraphData[]> = state => getVisualMetrics(state).PerceptualSpeedIndexProgress;
export const getContentfulSpeedIndexProgress: Selector<ProgressGraphData[]> = state => getVisualMetrics(state).ContentfulSpeedIndexProgress;
export const getProfilerConfiguration: Selector<ProfilerConfiguration | null | undefined> = state => getMeta(state).configuration;
...
export const getActiveBrowsingContextID: Selector<BrowsingContextID | null> = state => {
  ...
  if (configuration && configuration.activeBrowsingContextID && configuration.activeBrowsingContextID !== 0) {
 ...
 */
function _createCounterSelectors(counterIndex: CounterIndex): any {
  const getCounter: Selector<Counter> = state => ensureExists(getProfile(state).counters, 'Attempting to get a counter by index, but no counters exist.')[counterIndex];

  const getDescription: Selector<string> = state => getCounter(state).description;

  const getPid: Selector<Pid> = state => getCounter(state).pid;

  const getCommittedRangeFilteredCounter: Selector<Counter> = createSelector(getCounter, getCommittedRange, (counters, range) => filterCounterToRange(counters, range.start, range.end));

  const getAccumulateCounterSamples: Selector<Array<AccumulatedCounterSamples>> = createSelector(getCommittedRangeFilteredCounter, counters => accumulateCounterSamples(counters.sampleGroups.map(group => group.samples)));
...
export const getIPCMarkerCorrelations: Selector<IPCMarkerCorrelations> = createSelector(getThreads, correlateIPCMarkers);
 ...
 */
export const getGlobalTracks: Selector<GlobalTrack[]> = state => getFullProfileView(state).globalTracks;

/**
 * This returns all TrackReferences for global tracks.
 */
export const getGlobalTrackReferences: Selector<GlobalTrackReference[]> = createSelector(getGlobalTracks, globalTracks => globalTracks.map((globalTrack, trackIndex) => ({
  type: 'global',
  trackIndex
})));

export const getHasPreferenceMarkers: Selector<boolean> = createSelector(getThreads, threads => {
  return threads.some(({
    stringTable,
    markers
  }) => {
     ...
     */
    const indexForPreferenceString = stringTable.indexForString('PreferenceRead');
 ...
 */
export const getGlobalTrackFromReference: DangerousSelectorWithArguments<GlobalTrack, GlobalTrackReference> = (state, trackReference) => getGlobalTracks(state)[trackReference.trackIndex];
 ...
 */
export const getGlobalTrackAndIndexByPid: DangerousSelectorWithArguments<{readonly globalTrackIndex: TrackIndex;readonly globalTrack: GlobalTrack;}, Pid> = (state, pid) => {
  const globalTracks = getGlobalTracks(state);
  const globalTrackIndex = globalTracks.findIndex(track => track.type === 'process' && track.pid === pid);
 ...
 */
export const getLocalTracksByPid: Selector<Map<Pid, LocalTrack[]>> = state => getFullProfileView(state).localTracksByPid;
 ...
 */
export const getLocalTracks: DangerousSelectorWithArguments<LocalTrack[], Pid> = (state, pid) => ensureExists(getFullProfileView(state).localTracksByPid.get(pid), 'Unable to get the tracks for the given pid.');
 ...
 */
export const getLocalTrackFromReference: DangerousSelectorWithArguments<LocalTrack, LocalTrackReference> = (state, trackReference) => getLocalTracks(state, trackReference.pid)[trackReference.trackIndex];
 ...
 */
export const getProcessesWithMemoryTrack: Selector<Set<Pid>> = createSelector(getLocalTracksByPid, localTracksByPid => {
...
export const getRightClickedThreadIndex: Selector<null | ThreadIndex> = createSelector(getRightClickedTrack, getGlobalTracks, getLocalTracksByPid, (rightClickedTrack, globalTracks, localTracksByPid) => {
  ...
  const localTracks = ensureExists(localTracksByPid.get(pid), 'No local tracks found at that pid.');
  const track = localTracks[trackIndex];

  return track.type === 'thread' ? track.threadIndex : null;
});

export const getGlobalTrackNames: Selector<string[]> = createSelector(getGlobalTracks, getThreads, (globalTracks, threads) => globalTracks.map(globalTrack => Tracks.getGlobalTrackName(globalTrack, threads)));

export const getGlobalTrackName: DangerousSelectorWithArguments<string, TrackIndex> = (state, trackIndex) => getGlobalTrackNames(state)[trackIndex];

export const getLocalTrackNamesByPid: Selector<Map<Pid, string[]>> = createSelector(getLocalTracksByPid, getThreads, (localTracksByPid, threads) => {
  const localTrackNamesByPid = new Map();
  for (const [pid, localTracks] of localTracksByPid) {
    localTrackNamesByPid.set(pid, localTracks.map(localTrack => Tracks.getLocalTrackName(localTrack, threads)));
  }
  return localTrackNamesByPid;
});

export const getLocalTrackName = (state: State, pid: Pid, trackIndex: TrackIndex): string => ensureExists(getLocalTrackNamesByPid(state).get(pid), 'Could not find the track names from the given pid')[trackIndex];
 ...
 */
export const getActiveTabGlobalTracks: Selector<ActiveTabGlobalTrack[]> = state => getActiveTabProfileView(state).globalTracks;

/**
 * Returns resource tracks for the active tab view.
 */
export const getActiveTabResourceTracks: Selector<LocalTrack[]> = state => getActiveTabProfileView(state).resourceTracks;

/**
 * This returns all TrackReferences for global tracks.
 */
export const getActiveTabGlobalTrackReferences: Selector<GlobalTrackReference[]> = createSelector(getActiveTabGlobalTracks, globalTracks => globalTracks.map((globalTrack, trackIndex) => ({
  type: 'global',
  trackIndex
})));

export const getActiveTabHiddenGlobalTracksGetter: Selector<() => Set<TrackIndex>> = state => getActiveTabProfileView(state).hiddenGlobalTracksGetter;

export const getActiveTabHiddenLocalTracksByPidGetter: Selector<() => Map<Pid, Set<TrackIndex>>> = state => getActiveTabProfileView(state).hiddenLocalTracksByPidGetter;
 ...
 */
export const getHiddenTrackCount: Selector<HiddenTrackCount> = createSelector(getGlobalTracks, getLocalTracksByPid, UrlState.getHiddenLocalTracksByPid, getActiveTabHiddenLocalTracksByPidGetter, UrlState.getHiddenGlobalTracks, getActiveTabHiddenGlobalTracksGetter, UrlState.getShowTabOnly, (globalTracks, localTracksByPid, hiddenLocalTracksByPid, activeTabHiddenLocalTracksByPidGetter, hiddenGlobalTracks, activeTabHiddenGlobalTracksGetter, showTabOnly) => {
  let hidden = 0;
  let total = 0;
  ...
  for (const [pid, localTracks] of localTracksByPid) {
    ...
    const activeTabHiddenLocalTracks = // Do not call the getter if we are not in the single tab view.
    showTabOnly !== null ? activeTabHiddenLocalTracksByPidGetter().get(pid) || new Set() : new Set();
    const globalTrackIndex = globalTracks.findIndex(track => track.type === 'process' && track.pid === pid);
    if (globalTrackIndex === -1) {
      throw new Error('Unable to find a global track from the given pid.');
    }
    if (!hiddenLocalTracks) {
      throw new Error('Unable to find the hidden local tracks from the given pid');
    }
    ...
    if (hiddenGlobalTracks.has(globalTrackIndex)) {
      ...
      if (showTabOnly !== null) {
        ...
        hidden -= [...activeTabHiddenLocalTracks].filter(t => hiddenLocalTracks.has(t)).length;
      }
    }
  ...
  if (showTabOnly) {
    ...
    hidden += [...hiddenGlobalTracks].filter(t => !activeTabHiddenGlobalTracks.has(t)).length;
  } else {
 ...
 */
export const getPagesMap: Selector<Map<BrowsingContextID, Page[]> | null> = createSelector(getPageList, pageList => {
...
export const getRelevantPagesForActiveTab: Selector<Page[]> = createSelector(getPagesMap, getActiveBrowsingContextID, (pagesMap, activeBrowsingContextID) => {
  if (pagesMap === null || pagesMap.size === 0 || activeBrowsingContextID === null) {
 ...
 */
export const getInnerWindowIDSetByBrowsingContextID: Selector<Map<BrowsingContextID, Set<InnerWindowID>> | null> = createSelector(getPagesMap, pagesMap => {
  ...
  for (const [browsingContextID, pages] of pagesMap) {
    innerWindowIDSetByBrowsingContextID.set(browsingContextID, new Set(pages.map(page => page.innerWindowID)));
  }
 ...
 * active tab view.
 */
export const getRelevantInnerWindowIDsForActiveTab: Selector<Set<InnerWindowID>> = createSelector(getInnerWindowIDSetByBrowsingContextID, getActiveBrowsingContextID, (pagesMap, activeBrowsingContextID) => {
  if (pagesMap === null || pagesMap.size === 0 || activeBrowsingContextID === null) {
 ...
 */
export const getRelevantInnerWindowIDsForCurrentTab: Selector<Set<InnerWindowID>> = createSelector(UrlState.getShowTabOnly, getRelevantInnerWindowIDsForActiveTab, (showTabOnly, relevantPages) => {
  if (showTabOnly === null) {
    // Return an empty set if we want to see everything or that data is not there.
    return new Set();
  }
 ...
 */
export const getComputedHiddenGlobalTracks: Selector<Set<TrackIndex>> = createSelector(UrlState.getHiddenGlobalTracks, UrlState.getShowTabOnly, getActiveTabHiddenGlobalTracksGetter, (hiddenGlobalTracks, showTabOnly, activeTabHiddenGlobalTracksGetter) => {
  if (showTabOnly === null) {
  ...
  return new Set([...hiddenGlobalTracks, ...activeTabHiddenGlobalTracksGetter()]);
});
 ...
 * then merges those Sets depending on the active tab view status.
 */
export const getComputedHiddenLocalTracksByPid: Selector<Map<Pid, Set<TrackIndex>>> = createSelector(UrlState.getHiddenLocalTracksByPid, UrlState.getShowTabOnly, getActiveTabHiddenLocalTracksByPidGetter, (hiddenLocalTracksByPid, showTabOnly, activeTabHiddenLocalTracksByPidGetter) => {
  if (showTabOnly === null) {
  ...
  for (const [pid, localTracks] of activeTabHiddenLocalTracksByPidGetter()) {
    ...
    if (entry) {
      mergedHiddenLocalTracksByPid.set(pid, new Set([...entry, ...localTracks]));
    } else {
      mergedHiddenLocalTracksByPid.set(pid, new Set([...localTracks]));
    }
  }
 ...
 */
export const getComputedHiddenLocalTracks: DangerousSelectorWithArguments<Set<TrackIndex>, Pid> = (state, pid) => ensureExists(getComputedHiddenLocalTracksByPid(state).get(pid), 'Unable to get the tracks for the given pid.');
 ...
 */
export const getProfileFilterPageData: Selector<ProfileFilterPageData | null> = createSelector(getPageList, getRelevantInnerWindowIDsForCurrentTab, extractProfileFilterPageData);