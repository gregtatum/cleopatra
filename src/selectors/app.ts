...
import { getGlobalTracks, getLocalTracksByPid, getComputedHiddenGlobalTracks, getComputedHiddenLocalTracksByPid } from "./profile";
import { getZipFileState } from "./zipped-profiles";
import { assertExhaustiveCheck, ensureExists } from "../utils/flow";
import { TRACK_SCREENSHOT_HEIGHT, TRACK_NETWORK_HEIGHT, TRACK_MEMORY_HEIGHT, TRACK_IPC_HEIGHT, TRACK_PROCESS_BLANK_HEIGHT, TIMELINE_RULER_HEIGHT, TIMELINE_SETTINGS_HEIGHT, TRACK_VISUAL_PROGRESS_HEIGHT } from "../app-logic/constants";

import { TabSlug } from "../app-logic/tabs-handling";
import { AppState, AppViewState, UrlSetupPhase } from "../types/state";
...
export const getUrlSetupPhase: Selector<UrlSetupPhase> = state => getApp(state).urlSetupPhase;
export const getHasZoomedViaMousewheel: Selector<boolean> = state => {
  return getApp(state).hasZoomedViaMousewheel;
};
export const getIsSidebarOpen: Selector<boolean> = state => getApp(state).isSidebarOpenPerPanel[getSelectedTab(state)];
export const getPanelLayoutGeneration: Selector<number> = state => getApp(state).panelLayoutGeneration;
export const getLastVisibleThreadTabSlug: Selector<TabSlug> = state => getApp(state).lastVisibleThreadTabSlug;
export const getTrackThreadHeights: Selector<Array<ThreadIndex | void>> = state => getApp(state).trackThreadHeights;
export const getIsNewlyPublished: Selector<boolean> = state => getApp(state).isNewlyPublished;

export const getIsDragAndDropDragging: Selector<boolean> = state => getApp(state).isDragAndDropDragging;
export const getIsDragAndDropOverlayRegistered: Selector<boolean> = state => getApp(state).isDragAndDropOverlayRegistered;
 ...
 */
export const getTimelineHeight: Selector<null | CssPixels> = createSelector(getGlobalTracks, getLocalTracksByPid, getComputedHiddenGlobalTracks, getComputedHiddenLocalTracksByPid, getTrackThreadHeights, getShowTabOnly, (globalTracks, localTracksByPid, hiddenGlobalTracks, hiddenLocalTracksByPid, trackThreadHeights, showTabOnly) => {
  ...
  for (const [pid, localTracks] of localTracksByPid) {
    ...
    for (const [trackIndex, localTrack] of localTracks.entries()) {
      const hiddenLocalTracks = ensureExists(hiddenLocalTracksByPid.get(pid), 'Could not look up the hidden local tracks from the given PID');
          ...
          case 'thread':
            {
              // The thread tracks have enough complexity that it warrants measuring
              // them rather than statically using a value like the other tracks.
              const trackThreadHeight = trackThreadHeights[localTrack.threadIndex];
              if (trackThreadHeight === undefined) {
                // The height isn't computed yet, return.
                return null;
              }
 ...
 */
export const getIsNewProfileLoadAllowed: Selector<boolean> = createSelector(getView, getDataSource, getZipFileState, (view, dataSource, zipFileState) => {
  const appPhase = view.phase;
  const zipPhase = zipFileState.phase;
  const isLoading = (appPhase === 'INITIALIZING' && dataSource !== 'none') || zipPhase === 'PROCESS_PROFILE_FROM_ZIP_FILE';
  return !isLoading;
});