...
import { TIMELINE_MARGIN_LEFT, TIMELINE_MARGIN_RIGHT, JS_TRACER_MAXIMUM_CHART_ZOOM } from "../../app-logic/constants";
...
import { JsTracerTable, ThreadIndex, Profile } from "../../types/profile";
import { JsTracerTiming } from "../../types/profile-derived";
import { UnitIntervalOfProfileRange, CssPixels, StartEndRange } from "../../types/units";
import { PreviewSelection } from "../../types/actions";
...
type StateProps = {
  ...
  readonly stringTable: UniqueStringArray;
...
class JsTracerExpensiveChartImpl extends React.PureComponent<Props> {
  ...
  render() {
    ...
    return <JsTracerCanvas key={threadIndex} viewportProps={{
      timeRange,
      previewSelection,
      maxViewportHeight,
      viewportNeedsUpdate,
...
function viewportNeedsUpdate(prevProps: {readonly jsTracerTimingRows: JsTracerTiming[];}, newProps: {readonly jsTracerTimingRows: JsTracerTiming[];}) {
 ...
 */
const JsTracerExpensiveChart = explicitConnect<OwnProps, StateProps, DispatchProps>({
  mapStateToProps: (state, ownProps) => ({
    timeRange: getCommittedRange(state),
    stringTable: selectedThreadSelectors.getStringTable(state),
    threadIndex: getSelectedThreadIndex(state),
    previewSelection: getPreviewSelection(state),
    jsTracerTimingRows: ensureExists(ownProps.showJsTracerSummary ? selectedThreadSelectors.getExpensiveJsTracerLeafTiming(state) : selectedThreadSelectors.getExpensiveJsTracerTiming(state), 'The JS tracer information must exist when mounting this component')
  }),
 ...
 */
class JsTracerChartLoader extends React.PureComponent<ChartLoaderProps, ChartLoaderState> {
  ...
  render() {
    ...
    return this.state.readyToRenderExpensiveChart || !this._doFadeIn ? <JsTracerExpensiveChart doFadeIn={this._doFadeIn} jsTracerTable={jsTracerTable} showJsTracerSummary={showJsTracerSummary} /> : <div className="jsTracerLoader">
...
export default class JsTracerChart extends React.PureComponent<ChartProps> {
  ...
  render() {
    ...
    return <JsTracerChartLoader key={key} keyString={key} jsTracerTable={jsTracerTable} showJsTracerSummary={showJsTracerSummary} profile={profile} />;
  }
}