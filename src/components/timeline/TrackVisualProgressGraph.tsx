...
import { ProgressGraphData } from "../../types/profile";
...
class TrackVisualProgressCanvas extends React.PureComponent<CanvasProps> {
  ...
  drawCanvas(canvas: HTMLCanvasElement): void {
...

    {
      ...
      ctx.lineTo(deviceWidth * (progressGraphData[0].timestamp - rangeStart) / rangeLength + interval, deviceHeight);
      ctx.fill();
    }
  }
  ...
  render() {
    ...
    return <canvas className="timelineTrackVisualProgressCanvas" ref={this._takeCanvasRef} />;
  }
}
...
class TrackVisualProgressGraphImpl extends React.PureComponent<Props, State> {
  ...
  _onMouseMove = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (timeAtMouse < progressGraphData[0].timestamp || timeAtMouse > progressGraphData[progressGraphData.length - 1].timestamp + interval) {
  ...
  _renderTooltip(graphDataIndex: number): React.ReactNode {
    ...
    return <div className="timelineTrackVisualProgressTooltip">
  ...
  _renderVisualProgressDot(graphDataIndex: number): React.ReactNode {
    ...
    const rangeLength = rangeEnd - rangeStart;
    const left = width * (progressGraphData[graphDataIndex].timestamp - rangeStart) / rangeLength;
    ...
    return <div style={{ left, top, '--dot-color': BLUE_60 }} className="timelineTrackVisualProgressGraphDot" />;
  }
  ...
  render() {
    ...
    return <div className="timelineTrackVisualProgressGraph" onMouseMove={this._onMouseMove} onMouseLeave={this._onMouseLeave}>
        <TrackVisualProgressCanvas rangeStart={rangeStart} rangeEnd={rangeEnd} progressGraphData={progressGraphData} height={graphHeight} width={width} lineWidth={lineWidth} interval={interval} />
...
export const TrackVisualProgressGraph = explicitConnect<OwnProps, StateProps, DispatchProps>({
