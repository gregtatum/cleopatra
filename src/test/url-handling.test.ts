...
import { changeCallTreeSearchString, changeMarkersSearchString, changeNetworkSearchString, changeProfileName } from "../actions/profile-view";
import { changeSelectedTab, changeProfilesToCompare } from "../actions/app";
import { stateFromLocation, urlStateToUrlObject, urlFromState, CURRENT_URL_VERSION, upgradeLocationToCurrentVersion } from "../app-logic/url-handling";
import { blankStore } from "./fixtures/stores";
import { viewProfile, changeViewAndRecomputeProfileData } from "../actions/receive-profile";
import { Profile } from "../types/profile";
import getProfile from "./fixtures/profiles/call-nodes";
import queryString from "query-string";
import { getHumanReadableTracks, getProfileWithNiceTracks } from "./fixtures/profiles/tracks";
import { getProfileFromTextSamples, addActiveTabInformationToProfile } from "./fixtures/profiles/processed-profile";
import { selectedThreadSelectors } from "../selectors/per-thread";
import { uintArrayToString } from "../utils/uintarray-encoding";
import { getActiveTabGlobalTracks, getActiveTabResourceTracks } from "../selectors/profile";

function _getStoreWithURL(settings: {
  pathname?: string;
  search?: string;
  hash?: string;
...
describe('url handling tracks', function () {
  ...
  describe('global tracks', function () {
    it('creates tracks without any set search parameters', function () {
      ...
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', 'show [thread GeckoMain tab]', '  - show [thread DOM Worker]', '  - show [thread Style]']);
    });

    it('can reorder global tracks ', function () {
      const {
        getState
      } = initWithSearchParams('?globalTrackOrder=1-0');
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain tab]', '  - show [thread DOM Worker]', '  - show [thread Style]', 'show [thread GeckoMain process] SELECTED']);
    });

    it('can hide tracks', function () {
      const {
        getState
      } = initWithSearchParams('?hiddenGlobalTracks=1');
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', 'hide [thread GeckoMain tab]', '  - show [thread DOM Worker]', '  - show [thread Style]']);
    });
    ...
    it('will not accept invalid hidden threads', function () {
      const {
        getState
      } = initWithSearchParams('?hiddenGlobalTracks=0-8-2-a&thread=1');
      expect(urlStateReducers.getHiddenGlobalTracks(getState())).toEqual(new Set([0]));
    });
  });
  ...
  describe('local tracks', function () {
    it('can reorder local tracks ', function () {
      const {
        getState
      } = initWithSearchParams('?localTrackOrderByPid=222-1-0');
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', 'show [thread GeckoMain tab]', '  - show [thread Style]', '  - show [thread DOM Worker]']);
    });

    it('can hide local tracks ', function () {
      const {
        getState
      } = initWithSearchParams('?hiddenLocalTracksByPid=222-1');
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', 'show [thread GeckoMain tab]', '  - show [thread DOM Worker]', '  - hide [thread Style]']);
    });
    ...
    it('can select a local track without mixing track and thread indexes', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples('A', 'B', 'C');
      ...
      const {
        getState
      } = _getStoreWithURL( // In this search query, we want to hide the second local track of the
      ...
      { search: '?hiddenLocalTracksByPid=111-1&thread=1' }, profile);

      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process]', '  - show [thread DOM Worker] SELECTED', '  - hide [thread Style]']);
    });
  });
  ...
  describe('legacy thread information', function () {
    it('handles legacy thread ordering', function () {
      ...
      const {
        getState
      } = initWithSearchParams('?threadOrder=3-2-1-0');
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain tab]', '  - show [thread Style]', '  - show [thread DOM Worker]', 'show [thread GeckoMain process] SELECTED']);
    });
    ...
    it('handles legacy thread hiding', function () {
      ...
      const {
        getState
      } = initWithSearchParams('?hiddenThreads=0-2');
      expect(getHumanReadableTracks(getState())).toEqual(['hide [thread GeckoMain process]', 'show [thread GeckoMain tab] SELECTED', '  - hide [thread DOM Worker]', '  - show [thread Style]']);
    });
  });
});
...
describe('search strings', function () {
  it('properly handles the call tree search string stacks with 1 item', function () {
    const {
      getState
    } = _getStoreWithURL({ search: '?search=string' });
  ...
  it('properly handles the call tree search string stacks with several items', function () {
    ...
    expect(urlStateReducers.getCurrentSearchString(getState())).toBe('string,foo, bar');
    expect(urlStateReducers.getSearchStrings(getState())).toEqual(['string', 'foo', 'bar']);
  });
  ...
  it('properly handles marker search strings', function () {
    ...
    expect(urlStateReducers.getMarkersSearchString(getState())).toBe('otherString');
  });

  it('properly handles showUserTimings strings', function () {
    const {
      getState
    } = _getStoreWithURL({ search: '' });
...
describe('showTabOnly', function () {
  ...
  it('should remove other full view url states if present', function () {
    const {
      ...
      search: '?showTabOnly1=123&globalTrackOrder=3-2-1-0&hiddenGlobalTracks=4-5&hiddenLocalTracksByPid=111-1&thread=0'
    });

    const newUrl = new URL(urlFromState(urlStateReducers.getUrlState(getState())), 'https://profiler.firefox.com');
    // The url states that are relevant to full view should be stripped out.
    expect(newUrl.search).toEqual(`?showTabOnly1=123&thread=0&v=${CURRENT_URL_VERSION}`);
  });
});
...
describe('url upgrading', function () {
  ...
  describe('version 1: legacy URL serialized call tree filters', function () {
    it('can upgrade callTreeFilters to transforms', function () {
      const {
        ...
        search: '?callTreeFilters=prefix-012~prefixjs-123~postfix-234~postfixjs-345',
        v: false
      });
  ...
  describe('version 4: Add relevantForJs frames to JS callNodePaths', function () {
    it('can upgrade a simple stack with one relevantForJs frame in the middle', function () {
      ...
      profile.threads[0].funcTable.relevantForJS[funcNamesDictPerThread.DrelevantForJs] = true;

      const callNodePathBefore = [funcNamesDictPerThread['B.js'], funcNamesDictPerThread['C.js'], funcNamesDictPerThread['G.js']];
      ...
      const callNodePathAfter = [funcNamesDictPerThread['B.js'], funcNamesDictPerThread['C.js'], funcNamesDictPerThread.DrelevantForJs, funcNamesDictPerThread['G.js']];
    ...
    it('can upgrade a simple stack with one relevantForJs frame in the front', function () {
      ...
      profile.threads[0].funcTable.relevantForJS[funcNamesDictPerThread.BrelevantForJs] = true;

      const callNodePathBefore = [funcNamesDictPerThread['C.js'], funcNamesDictPerThread['E.js']];
      ...
      const callNodePathAfter = [funcNamesDictPerThread.BrelevantForJs, funcNamesDictPerThread['C.js'], funcNamesDictPerThread['E.js']];
    ...
    it('can upgrade a simple stack with relevantForJs and native frames in the middle', function () {
      ...
      const callNodePathBefore = [funcNamesDictPerThread['D.js'], funcNamesDictPerThread['F.js']];
      ...
      const callNodePathAfter = [funcNamesDictPerThread.BrelevantForJs, funcNamesDictPerThread['D.js'], funcNamesDictPerThread['F.js']];
    ...
    it('can upgrade the callNodePath in the second branch of the call tree', function () {
      ...
      profile.threads[0].funcTable.relevantForJS[funcNamesDictPerThread.CrelevantForJs] = true;

      const callNodePathBefore = [funcNamesDictPerThread['B.js'], funcNamesDictPerThread['E.js']];
      ...
      const callNodePathAfter = [funcNamesDictPerThread['B.js'], funcNamesDictPerThread.CrelevantForJs, funcNamesDictPerThread['E.js']];
  ...
  it("won't run if the version is specified", function () {
    ...
    expect(urlStateReducers.getSelectedTab(getState())).not.toBe('marker-table');
  });
});

describe('URL serialization of the transform stack', function () {
  const transformString = 'f-combined-012~mcn-combined-234~f-js-345-i~mf-6~ff-7~cr-combined-8-9~' + 'rec-combined-10~df-11~cfs-12';
  const {
    getState
  } = _getStoreWithURL({
  ...
  it('deserializes focus subtree transforms', function () {
    const transformStack = selectedThreadSelectors.getTransformStack(getState());
  ...
  it('re-serializes the focus subtree transforms', function () {
    const {
      query
    } = urlStateToUrlObject(urlStateReducers.getUrlState(getState()));
...
describe('urlFromState', function () {
  it('outputs no default parameters besides the current URL version', function () {
    const pathname = '/public/1ecd7a421948995171a4bb483b7bcc8e1868cc57/calltree';
    const newUrlState = stateFromLocation({
      pathname: pathname,
      search: '',
      hash: ''
    });
    expect(urlFromState(newUrlState)).toEqual(`${pathname}/?v=${CURRENT_URL_VERSION}`);
  });
});
...
describe('compare', function () {
  ...
  it('unserializes profiles URL properly', function () {
    ...
    expect(urlStateReducers.getProfilesToCompare(store.getState())).toEqual([url1, url2]);
  });

  it('serializes profiles URL properly', function () {
    const store = _getStoreWithURL({ pathname: '/compare/' },
    /* no profile */
    null);

    const initialUrl = urlFromState(urlStateReducers.getUrlState(store.getState()));
    expect(initialUrl).toEqual('/compare/');

    store.dispatch(changeProfilesToCompare([url1, url2]));
    const resultingUrl = urlFromState(urlStateReducers.getUrlState(store.getState()));
...
describe('last requested call tree summary strategy', function () {
  ...
  it('defaults to timing', function () {
    ...
    expect(getLastSelectedCallTreeSummaryStrategy(getState())).toEqual('timing');
  });
  ...
  it('can be js allocations', function () {
    ...
    expect(getLastSelectedCallTreeSummaryStrategy(getState())).toEqual('js-allocations');
  });
  ...
  it('can be native allocations', function () {
    ...
    expect(getLastSelectedCallTreeSummaryStrategy(getState())).toEqual('native-allocations');
  });
