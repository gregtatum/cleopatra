...
import { resourceTypes, getEmptyExtensions, getEmptyFuncTable, getEmptyResourceTable, getEmptyRawMarkerTable, getEmptyJsAllocationsTable, getEmptyUnbalancedNativeAllocationsTable } from "./data-structures";
import { immutableUpdate, ensureExists } from "../utils/flow";
import { upgradeProcessedProfileToCurrentVersion, isProcessedProfile } from "./processed-profile-versioning";
...
import { isPerfScriptFormat, convertPerfScriptProfile } from "./import/linux-perf";
import { convertPhaseTimes } from "./convert-markers";
import { PROCESSED_PROFILE_VERSION } from "../app-logic/constants";
import { getFriendlyThreadName, getOrCreateURIResource } from "../profile-logic/profile-data";
import { convertJsTracerToThread } from "../profile-logic/js-tracer";

import { Profile, Thread, Counter, ExtensionTable, CategoryList, FrameTable, SamplesTable, StackTable, RawMarkerTable, Lib, FuncTable, ResourceTable, IndexIntoStackTable, IndexIntoFuncTable, IndexIntoStringTable, IndexIntoResourceTable, JsTracerTable, JsAllocationsTable, ProfilerOverhead, NativeAllocationsTable } from "../types/profile";
import { Milliseconds, Microseconds } from "../types/units";
import { GeckoProfile, GeckoSubprocessProfile, GeckoThread, GeckoMarkerStruct, GeckoFrameStruct, GeckoSampleStruct, GeckoStackStruct, GeckoProfilerOverhead } from "../types/gecko-profile";
import { GCSliceMarkerPayload, GCMajorMarkerPayload, MarkerPayload, MarkerPayload_Gecko, GCSliceData_Gecko, GCMajorCompleted, GCMajorCompleted_Gecko, GCMajorAborted } from "../types/markers";
...
function _sortByField<T extends Object>(fieldName: string, geckoTable: T): T {
 ...
 */
export function extractFuncsAndResourcesFromFrameLocations(locationStringIndexes: IndexIntoStringTable[], relevantForJSPerFrame: boolean[], stringTable: UniqueStringArray, libs: Lib[], extensions: ExtensionTable = getEmptyExtensions()): [FuncTable, ResourceTable, IndexIntoFuncTable[]] {
  ...
  const locationFuncs = locationStringIndexes.map((locationIndex, frameIndex) => {
    ...
    if (funcIndex === null) {
      ...
      if (funcIndex === null) {
        ...
        if (funcIndex === null) {
          funcIndex = _extractUnknownFunctionType(extractionInfo, locationIndex, relevantForJS);
        }
      }
    }
 ...
 */
function _extractUnsymbolicatedFunction(extractionInfo: ExtractionInfo, locationString: string, locationIndex: IndexIntoStringTable): IndexIntoFuncTable | null {
 ...
 * present.
 */
function _extractCppFunction(extractionInfo: ExtractionInfo, locationString: string): IndexIntoFuncTable | null {
  // Check for a C++ location string.
  const cppMatch: RegExpResult = // Given:   "functionName (in library name) + 1234"
  // Captures: 1^^^^^^^^^^^     2^^^^^^^^^^^    3^^^
  /^(.*) \(in ([^)]*)\) (\+ [0-9]+)$/.exec(locationString) || // Given:   "functionName (in library name) (1234:1234)"
  // Captures: 1^^^^^^^^^^^     2^^^^^^^^^^^   3^^^^^^^^
  /^(.*) \(in ([^)]*)\) (\(.*:.*\))$/.exec(locationString) || // Given:   "functionName (in library name)"
  // Captures: 1^^^^^^^^^^^     2^^^^^^^^^^^
...
function _addExtensionOrigin(extractionInfo: ExtractionInfo, extensions: ExtensionTable, index: number): void {
  const {
 ...
 */
function _extractJsFunction(extractionInfo: ExtractionInfo, locationString: string): IndexIntoFuncTable | null {
  // Check for a JS location string.
  const jsMatch: RegExpResult = // Given:   "functionName (http://script.url/:1234:1234)"
  // Captures: 1^^^^^^^^^^  2^^^^^^^^^^^^^^^^^^ 3^^^ 4^^^
  /^(.*) \((.+?):([0-9]+)(?::([0-9]+))?\)$/.exec(locationString) || // Given:   "http://script.url/:1234:1234"
  // Captures: 2^^^^^^^^^^^^^^^^^ 3^^^ 4^^^
  ...
  const resourceIndex = getOrCreateURIResource(scriptURI, resourceTable, stringTable, originToResourceIndex);
...
function _extractUnknownFunctionType({
  funcTable
}: ExtractionInfo, locationIndex: IndexIntoStringTable, relevantForJS: boolean): IndexIntoFuncTable {
 ...
 */
function _processFrameTable(geckoFrameStruct: GeckoFrameStruct, funcTable: FuncTable, frameFuncs: IndexIntoFuncTable[]): FrameTable {
  return {
 ...
 */
function _processStackTable(geckoStackTable: GeckoStackStruct, frameTable: FrameTable, categories: CategoryList): StackTable {
 ...
 */
function _convertPayloadStackToIndex(data: MarkerPayload_Gecko): IndexIntoStackTable | null {
  if (!data) {
    return null;
  }
        ...
        case 'Native allocation':
          {
            ...
            inProgressNativeAllocations.stack.push(_convertPayloadStackToIndex(geckoPayload));
            inProgressNativeAllocations.length++;

            if (hasMemoryAddresses) {
              memoryAddress.push(ensureExists(geckoPayload.memoryAddress, 'Could not find the memoryAddress property on a gecko marker payload.'));
              threadId.push(ensureExists(geckoPayload.threadId, 'Could not find a threadId property on a gecko marker payload.'));
            }
            // Do not process the marker and add it to the marker list.
            continue;
          }
 ...
 * the GC information.
 */
function _processMarkerPayload(geckoPayload: MarkerPayload_Gecko): MarkerPayload {
  if (!geckoPayload) {
    return null;
  }
  ...
  switch (payload.type) {
    ...
    case 'GCMajor':
      {
        const geckoTimings: GCMajorAborted | GCMajorCompleted_Gecko = payload.timings;
...
function _processSamples(geckoSamples: GeckoSampleStruct): SamplesTable {
  ...
  if (geckoSamples.eventDelay) {
    ...
    throw new Error('The profile processor expected an eventDelay or responsiveness array in the samples table, but none was found.');
  }
 ...
 */
function _processCounters(geckoProfile: GeckoProfile | GeckoSubprocessProfile, // The counters are listed independently from the threads, so we need an index that
...
stableThreadList: Thread[], // The timing across processes must be normalized, this is the timing delta between
// various processes.
delta: Milliseconds): Counter[] {
  const geckoCounters = geckoProfile.counters;
  const mainThread = geckoProfile.threads.find(thread => thread.name === 'GeckoMain');
  ...
  const mainThreadIndex = stableThreadList.findIndex(thread => thread.name === 'GeckoMain' && thread.pid === mainThread.pid);

  if (mainThreadIndex === -1) {
    throw new Error('Unable to find the main thread in the stable thread list. This means that the ' + 'logic in the _processCounters function is wrong.');
  }

  return geckoCounters.reduce((result, {
    name,
    category,
    description,
    sample_groups
    ...
    const sampleGroups = sample_groups.map(sampleGroup => ({
      id: sampleGroup.id,
      samples: adjustTableTimestamps(_toStructOfArrays(sampleGroup.samples), delta)
    }));
 ...
 */
function _processProfilerOverhead(geckoProfile: GeckoProfile | GeckoSubprocessProfile, // The overhead data is listed independently from the threads, so we need an index that
// references back into a stable list of threads. The threads list in the processing
// step is built dynamically, so the "stableThreadList" variable is a hint that this
// should be a stable and sorted list of threads.
stableThreadList: Thread[], // The timing across processes must be normalized, this is the timing delta between
// various processes.
delta: Milliseconds): ProfilerOverhead | null {
  const geckoProfilerOverhead: GeckoProfilerOverhead | null | undefined = geckoProfile.profilerOverhead;
  const mainThread = geckoProfile.threads.find(thread => thread.name === 'GeckoMain');
  ...
  if (mainThreadIndex === -1) {
    throw new Error('Unable to find the main thread in the stable thread list. This means that the ' + 'logic in the _processProfilerOverhead function is wrong.');
  }

  return {
    samples: adjustProfilerOverheadTimestamps(_toStructOfArrays(geckoProfilerOverhead.samples), delta),
    pid: mainThread.pid,
    mainThreadIndex,
 ...
 * information.
 */
function _processThread(thread: GeckoThread, processProfile: GeckoProfile | GeckoSubprocessProfile, extensions: ExtensionTable): Thread {
  const geckoFrameStruct: GeckoFrameStruct = _toStructOfArrays(thread.frameTable);
  const geckoStackTable: GeckoStackStruct = _toStructOfArrays(thread.stackTable);
  const geckoSamples: GeckoSampleStruct = _toStructOfArrays(thread.samples);
  const geckoMarkers: GeckoMarkerStruct = _toStructOfArrays(_sortByField('time', thread.markers));
  ...
  const [funcTable, resourceTable, frameFuncs] = extractFuncsAndResourcesFromFrameLocations(geckoFrameStruct.location, geckoFrameStruct.relevantForJS, stringTable, libs, extensions);
  const frameTable: FrameTable = _processFrameTable(geckoFrameStruct, funcTable, frameFuncs);
  const stackTable = _processStackTable(geckoStackTable, frameTable, categories);
  const {
    markers,
    jsAllocations,
    nativeAllocations
  ...
  const newThread: Thread = {
    ...
    processName: typeof thread.processName === 'string' ? thread.processName : '',
  ...
  function processJsTracer() {
    ...
    if (jsTracerEvents && jsTracerDictionary) {
      ...
      const geckoToProcessedStringIndex: number[] = new Array(jsTracerDictionary.length);
      for (let i = 0; i < jsTracerDictionary.length; i++) {
        geckoToProcessedStringIndex[i] = newThread.stringTable.indexForString(jsTracerDictionary[i]);
      }
 ...
 */
export function adjustTableTimestamps<Table extends {time: Milliseconds[];}>(table: Table, delta: Milliseconds): Table {
  return {
    ...table,
 ...
 * we deal with the integrated profile.
 */
function _adjustJsTracerTimestamps(jsTracer: JsTracerTable, delta: Milliseconds): JsTracerTable {
 ...
 * into milliseconds.
 */
export function adjustProfilerOverheadTimestamps<Table extends {time: Microseconds[];}>(table: Table, delta: Milliseconds): Table {
  return {
    ...table,
 ...
 */
export function adjustMarkerTimestamps(markers: RawMarkerTable, delta: Milliseconds): RawMarkerTable {
  return {
    ...markers,
 ...
 */
export function processProfile(rawProfile: GeckoProfile | {profile: GeckoProfile;}): Profile {
  ...
  const extensions: ExtensionTable = geckoProfile.meta.extensions ? _toStructOfArrays(geckoProfile.meta.extensions) : getEmptyExtensions();
  ...
  const nullableProfilerOverhead: Array<ProfilerOverhead | null> = [_processProfilerOverhead(geckoProfile, threads, 0)];
  ...
  for (const subprocessProfile of geckoProfile.processes) {
    ...
    threads = threads.concat(subprocessProfile.threads.map(thread => {
      const newThread: Thread = _processThread(thread, subprocessProfile, extensions);
      newThread.samples = adjustTableTimestamps(newThread.samples, adjustTimestampsBy);
      newThread.markers = adjustMarkerTimestamps(newThread.markers, adjustTimestampsBy);
      if (newThread.jsTracer) {
        newThread.jsTracer = _adjustJsTracerTimestamps(newThread.jsTracer, adjustTimestampsBy);
      }
      if (newThread.jsAllocations) {
        newThread.jsAllocations = adjustTableTimestamps(newThread.jsAllocations, adjustTimestampsBy);
      }
      if (newThread.nativeAllocations) {
        newThread.nativeAllocations = adjustTableTimestamps(newThread.nativeAllocations, adjustTimestampsBy);
      }
    ...
    counters.push(..._processCounters(subprocessProfile, threads, adjustTimestampsBy));

    nullableProfilerOverhead.push(_processProfilerOverhead(subprocessProfile, threads, adjustTimestampsBy));
  }
  ...
  const profilerOverhead: ProfilerOverhead[] = nullableProfilerOverhead.reduce((acc, overhead) => {
    if (overhead !== null) {
      acc.push(overhead);
    }
    return acc;
  ...
  for (const thread of threads.slice()) {
    ...
    if (jsTracer) {
      ...
      const jsTracerThread = convertJsTracerToThread(thread, jsTracer, meta.categories);
 ...
 */
export async function unserializeProfileOfArbitraryFormat(stringOrObject: string | Object): Promise<Profile> {
  try {
    let profile = null;
