...
import { getEmptyExtensions, shallowCloneRawMarkerTable } from "./data-structures";
import { removeURLs } from "../utils/string";
import { removeNetworkMarkerURLs, removePrefMarkerPreferenceValues, filterRawMarkerTableToRangeWithMarkersToDelete } from "./marker-data";
import { filterThreadSamplesToRange } from "./profile-data";
import { Profile, Thread, ThreadIndex } from "../types/profile";
 ...
 */
export function sanitizePII(profile: Profile, maybePIIToBeRemoved: RemoveProfileInformation | null): SanitizeProfileResult {
  ...
  if (profile.pages) {
    if (PIIToBeRemoved.shouldRemoveUrls) {
      pages = profile.pages.map(page => Object.assign({}, page, {
        url: 'Page #' + urlCounter++
      }));
    } else {
      pages = profile.pages;
  ...
  const newProfile = {
    ...
    meta: {
      ...profile.meta,
      extensions: PIIToBeRemoved.shouldRemoveExtensions ? getEmptyExtensions() : profile.meta.extensions
    },
    pages: pages,
    threads: profile.threads.reduce((acc, thread, threadIndex) => {
      const newThread: Thread | null = sanitizeThreadPII(thread, threadIndex, PIIToBeRemoved);
    ...
    counters: profile.counters ? profile.counters.reduce((acc, counter) => {
    ...
    profilerOverhead: profile.profilerOverhead ? profile.profilerOverhead.reduce((acc, overhead) => {
      const newThreadIndex = oldThreadIndexToNew.get(overhead.mainThreadIndex);
  ...
  return {
    ...
    oldThreadIndexToNew: oldThreadIndexToNew.size === profile.threads.length ? null : oldThreadIndexToNew
  };
}
...
export function getShouldSanitizeByDefault(profile: Profile): boolean {
  switch (profile.meta.updateChannel) {
    case 'default':case 'nightly':case 'nightly-try':case 'nightly-autoland':
      // local build
      return false;
    default:
      return true;
 ...
 */
function sanitizeThreadPII(thread: Thread, threadIndex: number, PIIToBeRemoved: RemoveProfileInformation): Thread | null {
  ...
  if (PIIToBeRemoved.shouldRemoveUrls || PIIToBeRemoved.shouldRemovePreferenceValues || PIIToBeRemoved.shouldRemoveThreadsWithScreenshots.size > 0) {
    for (let i = 0; i < markerTable.length; i++) {
      ...
      if (PIIToBeRemoved.shouldRemovePreferenceValues && currentMarker && currentMarker.type && currentMarker.type === 'PreferenceRead') {
      ...
      if (PIIToBeRemoved.shouldRemoveUrls && currentMarker && currentMarker.type && currentMarker.type === 'Network') {
      ...
      if (PIIToBeRemoved.shouldRemoveThreadsWithScreenshots.has(threadIndex) && currentMarker && currentMarker.type && currentMarker.type === 'CompositorScreenshot') {
  ...
  if (markersToDelete.size > 0 || PIIToBeRemoved.shouldFilterToCommittedRange !== null) {
    // Filter marker table with given range and marker indexes array.
    markerTable = filterRawMarkerTableToRangeWithMarkersToDelete(markerTable, markersToDelete, PIIToBeRemoved.shouldFilterToCommittedRange).rawMarkerTable;
