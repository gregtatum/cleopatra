...
import { CategoryList } from "../types/profile";
import { DOMEventMarkerPayload, UserTimingMarkerPayload, MarkerPayload, TextMarkerPayload } from "../types/markers";
import { Marker, MarkerIndex, MarkerTiming, MarkerTimingAndBuckets } from "../types/profile-derived";
 ...
 */
export function getMarkerTimingAndBuckets(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], // Categories can be null for things like Network Markers, where we don't care to
// break things up by category.
categories: CategoryList | null | undefined): MarkerTimingAndBuckets {
...
export function getMarkerTiming(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[]): MarkerTiming[] {
  // Flow didn't understand the filter operation here, so filter out bucket names
  // imperatively.
  const onlyTiming = [];
  for (const timingOrString of getMarkerTimingAndBuckets(getMarker, markerIndexes)) {
...
function computeMarkerLabel(data: MarkerPayload): string {
  ...
  if (data !== null && typeof data === 'object') {
    ...
    switch (data.type) {
      case 'UserTiming':
        return (data as UserTimingMarkerPayload).name;
      case 'tracing':
        if (data.category === 'DOMEvent') {
          return (data as DOMEventMarkerPayload).eventType;
        }

        break;
      case 'Text':
        return (data as TextMarkerPayload).name;
      default:
    }
  }
