...
import { IDBFactory, IDBDatabase, IDBObjectStore, IDBIndex, IDBKeyRange } from "../types/indexeddb";
...
export type SymbolTableAsTuple = [Uint32Array, // addrs
Uint32Array, // index
Uint8Array // buffer
];
...
export default class SymbolStoreDB {
  ...
  _setupDB(dbName: string): Promise<IDBDatabase> {
    ...
    return new Promise((resolve, reject) => {
      ...
      openReq.onblocked = () => {
        reject(new Error('The symbol store database could not be upgraded because it is ' + 'open in another tab. Please close all your other profiler.firefox.com ' + 'tabs and refresh.'));
      };
      ...
      return new Promise((resolve, reject) => {
        ...
        req.onsuccess = () => {
          ...
          if (cursor) {
            ...
            reject(new SymbolsNotFoundError('The requested library does not exist in the database.', { debugName, breakpadId }));
          }
        };
      });
    });
  ...
  _deleteRecordsLastUsedBeforeDate(store: SymbolStore, beforeDate: Date, callback: () => void): void {
    const lastUsedDateIndex: IDBIndex<any, Date, any> = store.index('lastUsedDate');
    ...
    const cursorReq = lastUsedDateIndex.openCursor((range as IDBKeyRange<SymbolDateKey>));
  ...
  _count(store: SymbolStore, callback: (arg0: number) => void): void {
