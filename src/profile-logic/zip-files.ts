...
import JSZip, { JSZipFile } from "jszip";
...
export function createZipTable(zipEntries: JSZip): ZipFileTable {
  const fullPaths = objectEntries(zipEntries.files).filter(([_fileName, file]) => !file.dir).map(([fileName, _file]) => fileName);
  ...
  for (const fullPath of fullPaths) {
    ...
    const pathParts = fullPath.split('/') // Prevent any empty strings from double // or trailing slashes.
    .filter(part => part);
    ...
    for (let i = 0; i < pathParts.length; i++) {
      ...
      filesTable.file[index] = i + 1 === pathParts.length ? zipEntries.files[fullPath] : null;
...
export class ZipFileTree {
  ...
  _computeChildrenArray(parentIndex: IndexIntoZipFileTable | null): IndexIntoZipFileTable[] {
    const children = [];
    for (let childIndex = 0; childIndex < this._zipFileTable.length; childIndex++) {
  ...
  getAllDescendants(zipTableIndex: IndexIntoZipFileTable): Set<IndexIntoZipFileTable> {
  ...
  getDisplayData(zipTableIndex: IndexIntoZipFileTable): ZipDisplayData {
    ...
    if (displayData === undefined) {
      ...
      if (!this.hasChildren(zipTableIndex)) {
        url = window.location.origin + '/from-url/' + encodeURIComponent(this._zipFileUrl) + '/' + // Type check the slug:
        ensureIsValidTabSlug('calltree') + '/?file=' + encodeURIComponent(this._zipFileTable.path[zipTableIndex]);
      }
 ...
 */
export function procureInitialInterestingExpandedNodes(zipFileTree: ZipFileTree, maxExpandedNodes: number = 30) {
