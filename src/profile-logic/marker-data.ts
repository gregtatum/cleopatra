...
import { Thread, SamplesTable, RawMarkerTable, IndexIntoStringTable, IndexIntoRawMarkerTable, IndexIntoCategoryList, InnerWindowID } from "../types/profile";
import { Marker, MarkerIndex } from "../types/profile-derived";
import { IPCPairData, BailoutPayload, NetworkPayload, PrefMarkerPayload, InvalidationPayload } from "../types/markers";
 ...
 */
export function deriveJankMarkers(samples: SamplesTable, thresholdInMs: number, otherCategoryIndex: IndexIntoCategoryList): Marker[] {
  const addMarker = () => jankInstances.push({
...
export function getSearchFilteredMarkerIndexes(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], searchRegExp: RegExp | null): MarkerIndex[] {
  if (!searchRegExp) {
  ...
  for (const markerIndex of markerIndexes) {
    ...
    if (data && typeof data === 'object') {
      ...
      if (data.type === 'FileIO') {
        ...
        if (searchRegExp.test(filename) || searchRegExp.test(operation) || searchRegExp.test(source)) {
          newMarkers.push(markerIndex);
          continue;
        }
        ...
        if (searchRegExp.test(messageType) || searchRegExp.test(otherPid.toString())) {
          newMarkers.push(markerIndex);
          continue;
        }
      ...
      if (typeof data.eventType === 'string' && searchRegExp.test(data.eventType)) {
      ...
      if (typeof data.category === 'string' && searchRegExp.test(data.category)) {
        newMarkers.push(markerIndex);
        continue;
      }
    }
 ...
 */
export function getTabFilteredMarkerIndexes(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], relevantPages: Set<InnerWindowID>, includeGlobalMarkers: boolean = true): MarkerIndex[] {
 ...
 */
export function extractMarkerDataFromName(markers: RawMarkerTable, stringTable: UniqueStringArray): RawMarkerTable {
  ...
  const bailoutRegex = // Capture groups:
  ...
  const invalidateRegex = // Capture groups:
  //         url        line
  //           ↓        ↓
  /^Invalidate (.*?)(?::(\d+))?$/;
  ...
  for (let markerIndex = 0; markerIndex < markers.length; markerIndex++) {
    ...
    if (name.startsWith('Bailout_')) {
      ...
      if (!match) {
        ...
        const [, type, afterAt, where, bailoutLine, script, functionLine] = match;
    ...
    if (matchFound && markers.data[markerIndex]) {
      console.error("A marker's payload was rewritten based off the text content of the marker. " + "profiler.firefox.com assumed that the payload was empty, but it turns out it wasn't. " + 'This is most likely an error and should be fixed. The marker name is:', name);
    }
  }
...
export class IPCMarkerCorrelations {
  ...
  get(tid: number, index: number): IPCPairData | null | undefined {
...
export function deriveMarkersFromRawMarkerTable(rawMarkers: RawMarkerTable, stringTable: UniqueStringArray, threadId: number, threadRange: StartEndRange, ipcCorrelations: IPCMarkerCorrelations): Marker[] {
  ...
  const openTracingMarkers: Map<IndexIntoStringTable, MarkerIndex[]> = new Map();
      ...
      case 'tracing':
        {
          ...
          if (data.interval === 'start') {
            ...
            console.error(`'data.interval' holds the invalid value '${data.interval}' in marker index ${i}. This should not normally happen.`);
            matchedMarkers.push({
              start: time,
              dur: 0,
              name: stringTable.getString(name),
      ...
      case 'Network':
        {
          ...
          if (data.status === 'STATUS_START') {
            ...
            if (startIndex !== undefined) {
              ...
              const startData: NetworkPayload = (rawMarkers.data[startIndex] as any);
 ...
 */
export function filterRawMarkerTableToRange(markers: RawMarkerTable, rangeStart: number, rangeEnd: number): RawMarkerTable {
  const newMarkerTable = getEmptyRawMarkerTable();

  const filteredMarkerIndexesIter = filterRawMarkerTableToRangeIndexGenerator(markers, rangeStart, rangeEnd);
 ...
 */
export function* filterRawMarkerTableToRangeIndexGenerator(markers: RawMarkerTable, rangeStart: number, rangeEnd: number): Generator<MarkerIndex, void, void> {
  const isTimeInRange = (time: number): boolean => time < rangeEnd && time >= rangeStart;
  const intersectsRange = (start: number, end: number): boolean => start < rangeEnd && end >= rangeStart;

  // These maps contain the start markers we find while looping the marker
  // table.
  // The first map contains the start markers for tracing markers. They can be
  // nested and that's why we use an array structure as value.
  const openTracingMarkers: Map<IndexIntoStringTable, IndexIntoRawMarkerTable[]> = new Map();
      ...
      case 'Network':
        {
          ...
          if (data.status === 'STATUS_START') {
            ...
            if (startIndex !== undefined) {
              ...
              // We know this startIndex points to a Network marker.
              const startData: NetworkPayload = (markers.data[startIndex] as any);
              const endData = data;
  ...
  for (const startIndex of openNetworkMarkers.values()) {
    const data: NetworkPayload = (markers.data[startIndex] as any);
 ...
 */
export function filterRawMarkerTableToRangeWithMarkersToDelete(markerTable: RawMarkerTable, markersToDelete: Set<IndexIntoRawMarkerTable>, filterRange: StartEndRange | null): {
  ...
  const newMarkerTable = getEmptyRawMarkerTable();
  const oldMarkerIndexToNew: Map<IndexIntoRawMarkerTable, IndexIntoRawMarkerTable> = new Map();
  ...
  if (filterRange === null) {
    ...
    const filteredMarkerIndexIter = filterRawMarkerTableToRangeIndexGenerator(oldMarkers, start, end);
 ...
 */
export function filterMarkerIndexes(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], filterFunc: (arg0: Marker) => boolean): MarkerIndex[] {
  return markerIndexes.filter(markerIndex => {
    return filterFunc(getMarker(markerIndex));
  });
}

export function filterMarkerIndexesToRange(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], rangeStart: number, rangeEnd: number): MarkerIndex[] {
  return filterMarkerIndexes(getMarker, markerIndexes, marker => marker.start < rangeEnd && marker.start + marker.dur >= rangeStart);
}
...
export function isNavigationMarker({
  name,
  data
}: Marker) {
  if (name === 'TTI') {
...
export function isMemoryMarker(marker: Marker): boolean {
  ...
  return (data.type === 'GCMajor' || data.type === 'GCMinor' || data.type === 'GCSlice' || (data.type === 'tracing' && data.category === 'CC'));
}
...
export function filterForMarkerChart(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[]): MarkerIndex[] {
  return filterMarkerIndexes(getMarker, markerIndexes, marker => !isNetworkMarker(marker));
}

// Identifies mime type of a network request.
export function guessMimeTypeFromNetworkMarker(payload: NetworkPayload): string | null {
  let uri;
  try {
...
export function getColorClassNameForMimeType(mimeType: string | null): "network-color-css" | "network-color-js" | "network-color-html" | "network-color-img" | "network-color-other" {
  switch (mimeType) {
    case 'text/css':
...
export function groupScreenshotsById(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[]): Map<string, Marker[]> {
...
export function removeNetworkMarkerURLs(payload: NetworkPayload): NetworkPayload {
  return { ...payload, URI: '', RedirectURI: '' };
}

export function removePrefMarkerPreferenceValues(payload: PrefMarkerPayload): PrefMarkerPayload {
  ...
  if (marker.data) {
    ...
    switch (data.type) {
      ...
      case 'FileIO':
        ...
        if (data.filename) {
          description = data.operation ? `${description} — ${data.filename}` : data.filename;
        }
      ...
      case 'Log':
        ...
        break;
      case 'IPC':
        description = `${data.messageType} — ${data.direction === 'sending' ? 'sent to' : 'received from'} ${data.otherThreadName || data.otherPid}`;
        break;
      default:
    }
  }
