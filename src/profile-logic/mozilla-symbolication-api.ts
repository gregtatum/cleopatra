...
function _ensureIsAPIResult(result: any): APIResult {
  ...
  for (const jobResult of results) {
    if (!(jobResult instanceof Object) || !('found_modules' in jobResult) || !('stacks' in jobResult)) {
      throw new Error('Expected jobResult to have `found_modules` and `stacks` properties');
    }
    ...
    for (const stack of stacks) {
      ...
      for (const frameInfo of stack) {
        ...
        if (!('module_offset' in frameInfo) || !('module' in frameInfo) || !('frame' in frameInfo)) {
          throw new Error('Expected frameInfo to have `module_offset`, `module` and `frame` properties');
        }
      }
    }
  }
...
export function requestSymbols(requests: LibSymbolicationRequest[]): Array<Promise<Map<number, AddressResult>>> {
  const addressArrays = requests.map(({
    addresses
  }) => Array.from(addresses));
  const body = {
    memoryMap: requests.map(({
      lib: {
        debugName,
        breakpadId
      }
    }) => [debugName, breakpadId]),
    stacks: addressArrays.map((addressArray, libIndex) => addressArray.map(addr => [libIndex, addr]))
  };
  ...
  return requests.map(async function (request, libIndex) {
    ...
    try {
      ...
      throw new SymbolsNotFoundError('There was a problem with the JSON returned by the symbolication API.', lib, error);
    }

    if (!json.found_modules[`${debugName}/${breakpadId}`]) {
      throw new SymbolsNotFoundError(`The symbol server does not have symbols for ${debugName}/${breakpadId}.`, lib);
    }

    const addressInfo = json.stacks[libIndex];
    const addressArray = addressArrays[libIndex];
    if (addressInfo.length !== addressArray.length) {
      throw new SymbolsNotFoundError('The result from the symbol server has an unexpected length.', lib);
    }
    ...
    for (let i = 0; i < addressInfo.length; i++) {
      ...
      if (info.function !== undefined && info.function_offset !== undefined) {
        ...
        throw new SymbolsNotFoundError(`The result from the symbol server did not contain function information for address ${address}, even though found_modules was true for the library that this address belongs to`, lib);
      }
    }
    return results;
  });
