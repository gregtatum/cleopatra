...
import { Profile, Thread, ThreadIndex, Pid } from "../types/profile";
import { GlobalTrack, LocalTrack, TrackIndex } from "../types/profile-derived";
...
function _getDefaultLocalTrackOrder(tracks: LocalTrack[]) {
  ...
  // In place sort!
  trackOrder.sort((a, b) => LOCAL_TRACK_DISPLAY_ORDER[tracks[a].type] - LOCAL_TRACK_DISPLAY_ORDER[tracks[b].type]);
  return trackOrder;
}
...
function _getDefaultGlobalTrackOrder(tracks: GlobalTrack[]) {
  const trackOrder = tracks.map((_, index) => index);
  // In place sort!
  trackOrder.sort((a, b) => GLOBAL_TRACK_DISPLAY_ORDER[tracks[a].type] - GLOBAL_TRACK_DISPLAY_ORDER[tracks[b].type]);
  return trackOrder;
}
 ...
 */
export function initializeLocalTrackOrderByPid( // If viewing an existing profile, take the track ordering from the URL and sanitize it.
urlTrackOrderByPid: Map<Pid, TrackIndex[]> | null, // This is the list of the tracks.
localTracksByPid: Map<Pid, LocalTrack[]>, // If viewing an old profile URL, there were not tracks, only thread indexes. Turn
// the legacy ordering into track ordering.
legacyThreadOrder: ThreadIndex[] | null): Map<Pid, TrackIndex[]> {
  ...
  if (legacyThreadOrder === null) {
    ...
    for (const [pid, tracks] of localTracksByPid) {
      ...
      for (const threadIndex of legacyThreadOrder) {
        const trackIndex = tracks.findIndex(track => track.type === 'thread' && track.threadIndex === threadIndex);
        if (trackIndex !== -1) {
...
export function initializeHiddenLocalTracksByPid(urlHiddenTracksByPid: Map<Pid, Set<TrackIndex>> | null, localTracksByPid: Map<Pid, LocalTrack[]>, profile: Profile, legacyHiddenThreads: ThreadIndex[] | null): Map<Pid, Set<TrackIndex>> {
  ...
  for (const [pid, tracks] of localTracksByPid) {
    ...
    if (legacyHiddenThreads !== null) {
      ...
      for (let trackIndex = 0; trackIndex < tracks.length; trackIndex++) {
        ...
        if (track.type === 'thread' && _isThreadIdle(profile, profile.threads[track.threadIndex])) {
          hiddenTracks.add(trackIndex);
        }
      }
    } else {
 ...
 */
export function computeLocalTracksByPid(profile: Profile): Map<Pid, LocalTrack[]> {
  ...
  for (const localTracks of localTracksByPid.values()) {
    // In place sort!
    localTracks.sort((a, b) => LOCAL_TRACK_INDEX_ORDER[a.type] - LOCAL_TRACK_INDEX_ORDER[b.type]);
  }
...
export function computeGlobalTracks(profile: Profile): GlobalTrack[] {
  ...
  (a, b) => GLOBAL_TRACK_INDEX_ORDER[a.type] - GLOBAL_TRACK_INDEX_ORDER[b.type]);
 ...
 */
export function initializeGlobalTrackOrder( // This is the list of the tracks.
globalTracks: GlobalTrack[], // If viewing an existing profile, take the track ordering from the URL and sanitize it.
urlGlobalTrackOrder: TrackIndex[] | null, // If viewing an old profile URL, there were not tracks, only thread indexes. Turn
// the legacy ordering into track ordering.
legacyThreadOrder: ThreadIndex[] | null): TrackIndex[] {
  if (legacyThreadOrder !== null) {
    ...
    for (const threadIndex of legacyThreadOrder) {
      const trackIndex = globalTracks.findIndex(globalTrack => globalTrack.type === 'process' && globalTrack.mainThreadIndex === threadIndex);
      if (trackIndex !== -1) {
        trackOrder.push(trackIndex);
      }
    }
  ...
  if (urlGlobalTrackOrder !== null && urlGlobalTrackOrder.length !== globalTracks.length) {
  ...
  return urlGlobalTrackOrder !== null && _indexesAreValid(globalTracks.length, urlGlobalTrackOrder) ? urlGlobalTrackOrder : _getDefaultGlobalTrackOrder(globalTracks);
}

export function initializeSelectedThreadIndex(selectedThreadIndex: ThreadIndex | null, visibleThreadIndexes: ThreadIndex[], profile: Profile): ThreadIndex {
  if (selectedThreadIndex !== null && visibleThreadIndexes.includes(selectedThreadIndex)) {
  ...
  // order.
  const threadIndex = profile.threads.indexOf(_findDefaultThread(visibleThreadIndexes.map(threadIndex => profile.threads[threadIndex])));
  if (threadIndex === -1) {
...
export function initializeHiddenGlobalTracks(globalTracks: GlobalTrack[], profile: Profile, validTrackIndexes: TrackIndex[], urlHiddenGlobalTracks: Set<TrackIndex> | null, legacyHiddenThreads: ThreadIndex[] | null): Set<TrackIndex> {
  const hiddenGlobalTracks = new Set();

  if (legacyHiddenThreads !== null) {
    for (const threadIndex of legacyHiddenThreads) {
      const trackIndex = globalTracks.findIndex(track => track.type === 'process' && track.mainThreadIndex === threadIndex);
      if (trackIndex !== -1) {
...
export function getVisibleThreads(globalTracks: GlobalTrack[], hiddenGlobalTracks: Set<TrackIndex>, localTracksByPid: Map<Pid, LocalTrack[]>, hiddenTracksByPid: Map<Pid, Set<TrackIndex>>): ThreadIndex[] {
  const visibleThreads = [];
  for (let globalTrackIndex = 0; globalTrackIndex < globalTracks.length; globalTrackIndex++) {
    ...
    if (globalTrack.type === 'process') {
      ...
      const tracks = ensureExists(localTracksByPid.get(pid), 'A local track was expected to exist for the given pid.');
      const hiddenTracks = ensureExists(hiddenTracksByPid.get(pid), 'Hidden tracks were expected to exists for the given pid.');
      for (let localTrackIndex = 0; localTrackIndex < tracks.length; localTrackIndex++) {
...
export function getGlobalTrackName(globalTrack: GlobalTrack, threads: Thread[]): string {
    ...
    case 'process':
      {
        ...
        if (globalTrack.mainThreadIndex === null) {
          ...
          const processName = threads.filter(thread => thread.pid === pid).map(thread => thread.processName).find(processName => !!processName);
          if (processName) {
            return processName;
          }
        ...
        return getFriendlyThreadName(threads, threads[globalTrack.mainThreadIndex]);
      }
    case 'screenshots':
      return 'Screenshots';
...
export function getLocalTrackName(localTrack: LocalTrack, threads: Thread[]): string {
  switch (localTrack.type) {
    ...
    case 'memory':
      return 'Memory';
    case 'ipc':
      return `IPC — ${getFriendlyThreadName(threads, threads[localTrack.threadIndex])}`;
    default:
...
function _isThreadIdle(profile: Profile, thread: Thread): boolean {
  if ( // Don't hide the compositor.
  thread.name === 'Compositor' || ( // Don't hide the main thread of the parent process.
  thread.name === 'GeckoMain' && thread.processType === 'default') || ( // Don't hide the GPU thread on Windows.
  ...
  if (_isContentThreadWithNoPaint(thread)) {
    ...
    return _isThreadMostlyFullOfIdleSamples(profile, thread, PERCENTAGE_ACTIVE_SAMPLES_NON_PAINT);
  }
 ...
 */
function _isThreadMostlyFullOfIdleSamples(profile: Profile, thread: Thread, activeSamplePercentage: number = PERCENTAGE_ACTIVE_SAMPLES): boolean {
  ...
  for (let sampleIndex = 0; sampleIndex < thread.samples.length; sampleIndex++) {
    ...
    if (stackIndex === null) {
      ...
      maxActiveStackCount = activeSamplePercentage * (thread.samples.length - filteredStackCount);
    } else {
...
function _hasThreadAtLeastOneNonIdleSample(profile: Profile, thread: Thread): boolean {
...
function _findDefaultThread(threads: Thread[]): Thread | null {
  ...
  const contentThreadId = threads.findIndex(thread => thread.name === 'GeckoMain' && thread.processType === 'tab');
  const defaultThreadIndex = contentThreadId !== -1 ? contentThreadId : defaultThreadOrder(threads)[0];

  return threads[defaultThreadIndex];
}

export function isMainThread(thread: Thread): boolean {
  return (thread.name === 'GeckoMain' || // If the pid is a string, then it's not one that came from the system.
...
function _indexesAreValid(listLength: number, indexes: number[]) {
  return ( // The item length is valid.
  indexes.length === listLength && // The indexes are valid and include every single value.
  indexes.slice().sort().every((value, arrayIndex) => value === arrayIndex));
}