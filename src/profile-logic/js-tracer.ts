...
import { getEmptyFrameTable, getEmptyStackTable, getEmptySamplesTableWithEventDelay, getEmptyRawMarkerTable } from "./data-structures";
import { ensureExists } from "../utils/flow";
import { JsTracerTable, IndexIntoStringTable, IndexIntoJsTracerEvents, IndexIntoFuncTable, Thread, IndexIntoStackTable, SamplesTable, CategoryList } from "../types/profile";
 ...
 */
function getScriptLocationToFuncIndex(thread: Thread): ScriptLocationToFuncIndex {
  const {
    funcTable,
    stringTable
  } = thread;
 ...
 */
export function getJsTracerTiming(jsTracer: JsTracerTable, thread: Thread): JsTracerTiming[] {
  ...
  for (let tracerEventIndex = 0; tracerEventIndex < jsTracer.length; tracerEventIndex++) {
    ...
    if (column !== null && line !== null) {
      ...
      if (funcIndexInMap !== undefined) {
        if (funcIndexInMap === null) {
          ...
          displayName = `ƒ ${stringTable.getString(funcTable.name[funcIndex])}  ${displayName}`;
        }
      }
    }
 ...
 */
export function getJsTracerLeafTiming(jsTracer: JsTracerTable, stringTable: UniqueStringArray): JsTracerTiming[] {
  ...
  function reportSelfTime(tracerEventIndex: number, start: Microseconds, end: Microseconds): void {
    if (start === end) {
...

    {
      ...
      if (currEnd < currStart) {
        throw new Error(`currEnd < currStart "${displayName} - ${currEnd} < ${currStart}"`);
      }
      if (currStart < prevEnd) {
        throw new Error(`currStart < prevEnd "${displayName} - ${currStart} < ${prevEnd}"`);
      }
    }
  ...
  for (let currentEventIndex = 0; currentEventIndex < jsTracer.length; currentEventIndex++) {
    ...
    while (true) {
      ...
      if (prefixStart <= currentStart && prefixEnd > currentStart && prefixEnd < currentEnd) {
      ...
      if (prefixEnd <= currentStart) {
        ...
        const prefixName = stringTable.getString(jsTracer.events[prefixEventIndex]);
        const currentName = stringTable.getString(jsTracer.events[currentEventIndex]);
        ...
        console.error(`Prefix (parent) times for "${prefixName}": ${prefixStart}ms to ${prefixEnd}ms`);
        console.error(`Current (child) times for "${currentName}": ${currentStart}ms to ${currentEnd}ms`);
        throw new Error('The JS Tracer algorithm encountered some data it could not handle.');
      }
    }
  }
  ...
  for (; prefixesTip >= 0; prefixesTip--) {
    reportSelfTime(prefixesEventIndexes[prefixesTip], prefixesStarts[prefixesTip], prefixesEnds[prefixesTip]);
  }
 ...
 */
export function convertJsTracerToThreadWithoutSamples(fromThread: Thread, jsTracer: JsTracerFixed, categories: CategoryList): {
  thread: Thread;
  ...
  const implementationMap: Map<IndexIntoJsTracerEvents, IndexIntoStringTable | null> = new Map();
  ...
  const eventNameToImplementationStringIndex = {
    Interpreter: stringTable.indexForString(('interpreter' as JsImplementation)),
    Baseline: stringTable.indexForString(('baseline' as JsImplementation)),
    IonMonkey: stringTable.indexForString(('ion' as JsImplementation))
  };
  const frameEventTypes = new Set(Object.keys(eventNameToImplementationStringIndex));
  ...
  for (let tracerEventIndex = 0; tracerEventIndex < jsTracer.length; tracerEventIndex++) {
    ...
    if (frameEventTypes.has(eventName)) {
      ...
      if (prefixImplementation === undefined) {
        throw new Error(`Expected to find an implementation from a js tracer prefix index prefixIndex: ${prefixIndex}`);
      }
...
export function getJsTracerFixed(jsTracer: JsTracerTable): JsTracerFixed {
  ...
  for (let eventIndex = 1; eventIndex < jsTracer.length; eventIndex++) {
    ...
    if (currStart < prevStart) {
      ...
      if (currEnd < prevEnd) {// No adjustment needed
        // prev: [-----]
        // curr:  [--]
      } else {
        if (prevEnd <= currStart) {// There is no need to adjust the timing.
          // prev: [----]
          // curr:         [----]
        } else {
 ...
 * what is going on.
 */
export function convertJsTracerToThread(fromThread: Thread, jsTracer: JsTracerTable, categories: CategoryList): Thread {
  const jsTracerFixed = getJsTracerFixed(jsTracer);
  const {
    thread,
    stackMap
  } = convertJsTracerToThreadWithoutSamples(fromThread, jsTracerFixed, categories);
  thread.samples = getSelfTimeSamplesFromJsTracer(thread, jsTracerFixed, stackMap);
  return thread;
}
 ...
 */
export function getSelfTimeSamplesFromJsTracer(thread: Thread, jsTracer: JsTracerFixed, stackMap: Map<IndexIntoJsTracerEvents, IndexIntoStackTable>): SamplesTable {
  ...
  function addSelfTimeAsASample(eventIndex: IndexIntoJsTracerEvents, start: Microseconds, end: Microseconds): void {
    // Use a check against the epsilon, for float precision issues.
    if (isNearlyEqual(start, end)) {
      // If this self time is 0, do not report it.
      return;
    }
    const stackIndex = ensureExists(stackMap.get(eventIndex), 'The JS tracer event did not exist in the stack map.');
    samples.stack.push(stackIndex);
    samples.time.push( // Convert from microseconds.
    start / 1000);
    sampleWeights.push( // The weight of the sample is in microseconds.
    (end - start) / 1000);
    samples.length++;
  }
  // Determine the self time of the various events.
  ...
  for (let currentEventIndex = 0; currentEventIndex < jsTracer.length; currentEventIndex++) {
    ...
    while (true) {
      ...
      if (prefixEnd <= currentStart + epsilon) {
        ...
        console.error(`Prefix (parent) times for "${prefixName}": ${prefixStart}ms to ${prefixEnd}ms`);
        console.error(`Current (child) times for "${currentName}": ${currentStart}ms to ${currentEnd}ms`);
        throw new Error('The JS Tracer information was malformed. Some event lasted longer than its parent event.');
      }
    }
  }
  ...
  // Drain off the remaining "prefixes" from the stack, and report the self time.
  for (; prefixesTip >= 0; prefixesTip--) {
    addSelfTimeAsASample(prefixesEventIndexes[prefixesTip], prefixesStarts[prefixesTip], prefixesEnds[prefixesTip]);
  }
